...
success:function(data) {
var error, error_msg;
return data.status ? $(document).redirectPage(document.location.protocol + "//" + document.location.host + "/x/login?utm_source=hr_login") :(error = null, 
data && data.errors && data.errors.length > 0 && (error = data.errors[0]), error_msg = "", 
error_msg = error ? "&signup_error=" + encodeURI(error) :"", $(document).redirectPage(document.location.protocol + "//" + document.location.host + "/x/login?utm_source=hr_login&email=" + encodeURI(login) + error_msg));
},
error:function() {
return $(document).redirectPage(document.location.protocol + "//" + document.location.host + "/x/login?utm_source=hr_login&email=" + encodeURI(login) + "&signup_error=" + encodeURI("Unknown Error!"));
}
});
