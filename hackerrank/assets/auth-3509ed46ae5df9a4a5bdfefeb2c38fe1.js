...
if ($(e.currentTarget).removeClass("disabled"), fallback && data.hrx_user) return $(e.currentTarget).html("Log In"), 
...
success:function(data) {
return document.location.href = data.status ? document.location.protocol + "//" + document.location.host + "/x/login?utm_source=hr_login" :document.location.protocol + "//" + document.location.host + "/x/login?utm_source=hr_login&email=" + encodeURI(login) + "&signup_error=" + encodeURI(data.errors[0]);
},
error:function() {
