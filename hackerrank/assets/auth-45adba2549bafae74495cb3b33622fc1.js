...
return function() {
var events;
return $.fn.getVars = function(name) {
...
var continue_path;
return continue_path = $(document).getVars("continue"), continue_path ? document.location.href = document.location.protocol + "//" + document.location.host + continue_path :"reload" === path ? document.location.reload() :document.location.href = path;
}, events = function() {
...
$(document).redirectPage("reload")) :$(document).redirectPage(document.location.href.replace("/login", "/not_started").replace("/signup", "/not_started")) :($(e.currentTarget).html("Sign Up & Start Coding"), 
...
success:function(data) {
var error, error_msg;
return data.status ? $(document).redirectPage(document.location.protocol + "//" + document.location.host + "/work/login?utm_source=hr_login") :(error = null, 
data && data.errors && data.errors.length > 0 && (error = data.errors[0]), error_msg = "", 
error_msg = error ? "&signup_error=" + encodeURI(error) :"", $(document).redirectPage(document.location.protocol + "//" + document.location.host + "/work/login?utm_source=hr_login&email=" + encodeURI(login) + error_msg));
},
error:function() {
return $(document).redirectPage(document.location.protocol + "//" + document.location.host + "/work/login?utm_source=hr_login&email=" + encodeURI(login) + "&signup_error=" + encodeURI("Unknown Error!"));
}
});
...
if (data.status) return $(e.currentTarget).html("Logging you in..."), data.contest_started ? $(document).redirectPage("reload") :$(document).redirectPage(document.location.href.replace("/login", "/not_started").replace("/signup", "/not_started"));
...
$(document).redirectPage(document.location.protocol + "//" + document.location.host + login + contest);
}) :$(document).redirectPage(document.location.href);
