...
HR.connectionAlert && HR.connectionAlert(), "Template `" + that.templatePath("" + template_name + ".html") + "` Not Found";
},
...
return null == sbview && (sbview = null), sbview ? (this.sidebarview = sbview, this.$("#side-navigation").html(this.sidebarview.render().el), 
this.sidebarview.delegateEvents()) :this.$("#side-navigation").empty();
...
HR.candidate.showFocusMessage = !1, HR.candidate.lastQuestionViewed = null, HR.candidate.navLinksShouldWork = !0, 
...
return HR.requires("compound/recruit-candidate", function() {
var test;
return Offline.check(), HR.showThrobber(), test = new HR.CandidateTestModel(), test.setTidAuth(unique_id, HR.authkey), 
test.setAction("show"), test.fetch({
success:function() {
return function(m) {
var aid, attempt, dat;
return document.title = m.get("name") + " :: powered by HackerRank", HR.candidate.candidateTestModel = m, 
aid = HR.candidate.candidateTestModel.get("attempt"), m.get("unique_id") !== unique_id ? (HR.hideThrobber(1e3), 
...
attempt.set(HR.candidate.candidateTestModel.get("attempt_object")), HR.hideThrobber(1e3), 
HR.candidate.attemptRefreshNeeded = !1, HR.candidate.candidateAttemptModel = attempt, 
HR.redirect(test, attempt)) :(attempt = new HR.CandidateAttemptModel(), attempt.setAid(aid), 
dat = HR.candidate.attemptData ? HR.candidate.attemptData :{}, attempt.fetch({
data:dat,
processData:!0,
success:function(m) {
...
error:function() {
return function() {
return HR.hideThrobber(), HR.candidate.candidateTestModel = null, window.candidatemessage = "Unable to receive test data from this URL.", 
HR.router.navigate("" + unique_id + "/message", {
trigger:!0,
replace:!0
});
...
return HR.util.alert({
title:"Resource retrieve error",
message:"We were unable to load resources.<br><br>This is usually caused by an unstable internet connection. Try to refresh the page, or switch to a computer with a stable internet connection."
});
}, requirejs.onError = function() {
return Offline.check(), HR.connectionAlert();
