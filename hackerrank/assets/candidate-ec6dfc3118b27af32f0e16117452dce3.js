...
return HR.candidate.candidateTestModel = m, tid = m.get("unique_id"), m.get("logged_in") ? (m.get("logged_in") && !m.get("attempt_done") ? HR.candidate.redirectBackTo ? (console.log("navigating to " + HR.candidate.redirectBackTo), 
path = HR.candidate.redirectBackTo, HR.candidate.redirectBackTo = null, HR.router.navigate(path, {
trigger:!0,
replace:!0
...
attempt.setAid(HR.candidate.candidateTestModel.get("attempt")), dat = {
qlist:!0
}, attempt.fetch({
data:dat,
processData:!0,
...
model:HR.candidate.candidateTestModel
}), HR.candidate.currentQuestion = null, HR.appView.setTopbarView(HR.RecruitCandidateTopBarView), 
HR.appView.setSidebarView(HR.RecruitCandidateSideBarView), HR.appView.setContentView(candidate_view);
}, CandidateRouter;
