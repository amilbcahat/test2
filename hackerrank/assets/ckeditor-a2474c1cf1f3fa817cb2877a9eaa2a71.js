...
if (U) for (;M = U.pop(); ) CKEDITOR.env.needsBrFiller ? M.appendBogus() :M.append(N.document.createText(" "));
...
h = k.getDocument().createText("​"), k.setCustomData("cke-fillingChar", h), g.insertNode(h), 
(k = h.getNext()) && !h.getPrevious() && k.type == CKEDITOR.NODE_ELEMENT && "br" == k.getName() ? (j(this.root), 
...
}, (v = p.next()) && v.type == CKEDITOR.NODE_ELEMENT && v.is("ul", "ol") && (CKEDITOR.env.needsBrFiller ? o.createElement("br") :o.createText(" ")).insertBefore(v)), 
...
for (z = c = y[i]; (z = z.getNext()) && z.is && z.getName() in h; ) CKEDITOR.env.needsNbspFiller && !c.getFirst(a) && c.append(l.document.createText(" ")), 
c.append(z);
c.insertAfter(x);
}
...
n = CKEDITOR.plugins.list.arrayToList(a, null, n, d, C != z ? z :null), !y.getChildCount() && CKEDITOR.env.needsNbspFiller && !(f.$.documentMode > 7) && y.append(f.createText(" ")), 
...
init:function(b) {
for (var j = b.config.enableTabKeyTools !== !1, g = b.config.tabSpaces || 0, h = ""; g--; ) h += " ";
