...
for (var updated, oldFrom = cm.display.viewFrom, oldTo = cm.display.viewTo, visible = visibleLines(cm.display, cm.doc, viewPort), first = !0; ;first = !1) {
var oldWidth = cm.display.scroller.clientWidth;
if (!updateDisplayInner(cm, visible, forced)) break;
updated = !0, cm.display.maxLineChanged && !cm.options.lineWrapping && adjustContentWidth(cm);
var barMeasure = measureForScrollbars(cm);
if (updateSelection(cm), setDocumentHeight(cm, barMeasure), updateScrollbars(cm, barMeasure), 
webkit && cm.options.lineWrapping && checkForWebkitWidthBug(cm, barMeasure), webkit && barMeasure.scrollWidth > barMeasure.clientWidth && barMeasure.scrollWidth < barMeasure.clientWidth + 1 && !hScrollbarTakesSpace(cm) && updateScrollbars(cm), 
first && cm.options.lineWrapping && oldWidth != cm.display.scroller.clientWidth) forced = !0; else if (forced = !1, 
viewPort && null != viewPort.top && (viewPort = {
top:Math.min(barMeasure.docHeight - scrollerCutOff - barMeasure.clientHeight, viewPort.top)
}), visible = visibleLines(cm.display, cm.doc, viewPort), visible.from >= cm.display.viewFrom && visible.to <= cm.display.viewTo) break;
}
return cm.display.updateLineNumbers = null, updated && (signalLater(cm, "update", cm), 
(cm.display.viewFrom != oldFrom || cm.display.viewTo != oldTo) && signalLater(cm, "viewportChange", cm, cm.display.viewFrom, cm.display.viewTo)), 
updated;
}
function updateDisplayInner(cm, visible, forced) {
var display = cm.display, doc = cm.doc;
if (!display.wrapper.offsetWidth) return resetView(cm), void 0;
if (!(!forced && visible.from >= display.viewFrom && visible.to <= display.viewTo && (null == display.updateLineNumbers || display.updateLineNumbers >= display.viewTo) && 0 == countDirtyView(cm))) {
maybeUpdateLineNumberWidth(cm) && resetView(cm);
var dims = getDimensions(cm), end = doc.first + doc.size, from = Math.max(visible.from - cm.options.viewportMargin, doc.first), to = Math.min(end, visible.to + cm.options.viewportMargin);
...
if (different || 0 != toUpdate || forced) {
var focused = activeElt();
return toUpdate > 4 && (display.lineDiv.style.display = "none"), patchDisplay(cm, display.updateLineNumbers, dims), 
toUpdate > 4 && (display.lineDiv.style.display = ""), focused && activeElt() != focused && focused.offsetHeight && focused.focus(), 
removeChildren(display.cursorDiv), removeChildren(display.selectionDiv), different && (display.lastSizeC = display.wrapper.clientHeight, 
startWorker(cm, 400)), updateHeightsInViewport(cm), !0;
}
}
}
function adjustContentWidth(cm) {
var display = cm.display, width = measureChar(cm, display.maxLine, display.maxLine.text.length).left;
display.maxLineChanged = !1;
var minWidth = Math.max(0, width + 3), maxScrollLeft = Math.max(0, display.sizer.offsetLeft + minWidth + scrollerCutOff - display.scroller.clientWidth);
display.sizer.style.minWidth = minWidth + "px", maxScrollLeft < cm.doc.scrollLeft && setScrollLeft(cm, Math.min(display.scroller.scrollLeft, maxScrollLeft), !0);
}
...
for (var display = cm.display, doc = cm.doc, curFragment = document.createDocumentFragment(), selFragment = document.createDocumentFragment(), i = 0; i < doc.sel.ranges.length; i++) {
...
var headPos = cursorCoords(cm, doc.sel.primary().head, "div"), wrapOff = display.wrapper.getBoundingClientRect(), lineOff = display.lineDiv.getBoundingClientRect(), top = Math.max(0, Math.min(display.wrapper.clientHeight - 10, headPos.top + lineOff.top - wrapOff.top)), left = Math.max(0, Math.min(display.wrapper.clientWidth - 10, headPos.left + lineOff.left - wrapOff.left));
display.inputDiv.style.top = top + "px", display.inputDiv.style.left = left + "px";
}
...
var end = +new Date() + cm.options.workTime, state = copyState(doc.mode, getStateBefore(cm, doc.frontier));
runInOp(cm, function() {
...
ischange && regLineChange(cm, doc.frontier, "text"), line.stateAfter = copyState(doc.mode, state);
...
if (cm.curOp = null, op.updateMaxLine && findMaxLine(cm), op.viewChanged || op.forceUpdate || null != op.scrollTop || op.scrollToPos && (op.scrollToPos.from.line < display.viewFrom || op.scrollToPos.to.line >= display.viewTo) || display.maxLineChanged && cm.options.lineWrapping) {
...
if (!updated && op.selectionChanged && updateSelection(cm), updated || op.startHeight == cm.doc.height || updateScrollbars(cm), 
...
op.changeObjs && signal(cm, "changes", cm, op.changeObjs), delayed) for (var i = 0; i < delayed.length; ++i) delayed[i]();
if (op.cursorActivityHandlers) for (var i = 0; i < op.cursorActivityHandlers.length; i++) op.cursorActivityHandlers[i](cm);
}
...
}), on(d.input, "keyup", operation(cm, onKeyUp)), on(d.input, "input", function() {
...
Math.abs(cm.doc.scrollTop - val) < 2 || (cm.doc.scrollTop = val, gecko || updateDisplay(cm, {
top:val
...
function onKeyUp(e) {
signalDOMEvent(this, e) || 16 == e.keyCode && (this.doc.sel.shift = !1);
}
function onKeyPress(e) {
var cm = this;
if (!(signalDOMEvent(cm, e) || e.ctrlKey || mac && e.metaKey)) {
...
top:0,
bottom:0
}, nextOpId = 0, lastDrop = 0, wheelSamples = 0, wheelPixelsPerUnit = null;
...
eventMixin(Doc);
var delayedCallbacks, e_preventDefault = CodeMirror.e_preventDefault = function(e) {
...
}, delayedCallbackDepth = 0, scrollerCutOff = 30, Pass = CodeMirror.Pass = {
toString:function() {
...
return CodeMirror.version = "4.3.0", CodeMirror;
...
return stream.eat("=") ? "operator" :chain(readQuoted(ch, "string-2", !0), stream, state);
}
...
token:function(stream, state) {
if (null == state.indentStack && stream.sol() && (state.indentation = 6), stream.eatSpace()) return null;
var returnType = null;
switch (state.mode) {
case "string":
for (var next = !1; null != (next = stream.next()); ) if ('"' == next || "'" == next) {
state.mode = !1;
break;
}
returnType = STRING;
...
default:
var ch = stream.next(), col = stream.column();
if (col >= 0 && 5 >= col) returnType = COBOLLINENUM; else if (col >= 72 && 79 >= col) stream.skipToEnd(), 
returnType = MODTAG; else if ("*" == ch && 6 == col) stream.skipToEnd(), returnType = COMMENT; else if ('"' == ch || "'" == ch) state.mode = "string", 
returnType = STRING; else if ("'" != ch || tests.digit_or_colon.test(stream.peek())) if ("." == ch) returnType = PERIOD; else if (isNumber(ch, stream)) returnType = NUMBER; else {
if (stream.current().match(tests.symbol)) for (;71 > col && void 0 !== stream.eat(tests.symbol); ) col++;
returnType = keywords && keywords.propertyIsEnumerable(stream.current().toUpperCase()) ? KEYWORD :builtins && builtins.propertyIsEnumerable(stream.current().toUpperCase()) ? BUILTIN :atoms && atoms.propertyIsEnumerable(stream.current().toUpperCase()) ? ATOM :null;
} else returnType = ATOM;
}
return returnType;
},
...
var selectionStart = vim.marks["<"].find() || cm.getCursor("anchor"), selectionEnd = vim.marks[">"].find() || cm.getCursor("head");
vim.lastPastedText && (selectionEnd = cm.posFromIndex(cm.indexFromPos(selectionStart) + vim.lastPastedText.length - 1), 
vim.lastPastedText = null), vim.lastSelection = {
curStartMark:cm.setBookmark(selectionStart),
curEndMark:cm.setBookmark(selectionEnd),
visualMode:vim.visualMode,
visualLine:vim.visualLine
}, cursorIsBefore(selectionEnd, selectionStart) && (vim.lastSelection.curStartMark = cm.setBookmark(selectionEnd), 
...
updateLastSelection(cm, vim), vim.visualMode = !1, vim.visualLine = !1, cursorEqual(selectionStart, selectionEnd) || cm.setCursor(clipCursorToContent(cm, selectionEnd)), 
...
curEnd.ch -= 1) :cursorIsBefore(selectionEnd, selectionStart) && (cursorEqual(selectionStart, curEnd) || cursorIsBefore(selectionStart, curEnd)) && (selectionStart.ch -= 1, 
curEnd.ch += 1), 1/0 != vim.lastHPos && (vim.lastHPos = curEnd.ch), selectionEnd = curEnd, 
selectionStart = motionResult instanceof Array ? curStart :selectionStart, vim.visualLine) if (cursorIsBefore(selectionStart, selectionEnd)) {
selectionStart.ch = 0;
...
curEnd = selectionEnd, motionArgs.inclusive = !0), curEnd && cursorIsBefore(curEnd, curStart)) {
var tmp = curStart;
...
var curEnd = copyCursor(cm.getCursor("head")), curStart = copyCursor(cm.getCursor("anchor"));
return cursorIsBefore(curStart, curEnd) ? curEnd.ch += 1 :cursorIsBefore(curEnd, curStart) && (curStart.ch -= 1), 
[ curEnd, curStart ];
},
...
vim.visualLine = !!actionArgs.linewise, vim.visualLine ? (curStart.ch = 0, curEnd = clipCursorToContent(cm, Pos(curStart.line + repeat - 1, lineLength(cm, curStart.line)), !0)) :curEnd = clipCursorToContent(cm, Pos(curStart.line, curStart.ch + repeat), !0), 
...
if (lastSelection) {
var curStart = lastSelection.curStartMark.find(), curEnd = lastSelection.curEndMark.find();
if (cm.setSelection(curStart, curEnd), vim.visualMode) {
updateLastSelection(cm, vim);
var selectionStart = cm.getCursor("anchor"), selectionEnd = cm.getCursor("head");
updateMark(cm, vim, "<", cursorIsBefore(selectionStart, selectionEnd) ? selectionStart :selectionEnd), 
updateMark(cm, vim, ">", cursorIsBefore(selectionStart, selectionEnd) ? selectionEnd :selectionStart);
}
lastSelection.visualLine ? (vim.visualMode = !0, vim.visualLine = !0) :(vim.visualMode = !0, 
vim.visualLine = !1), CodeMirror.signal(cm, "vim-mode-change", {
mode:"visual",
subMode:vim.visualLine ? "linewise" :""
});
...
var curType, range = ranges[i], cur = range.head;
if ("'" == left && "comment" == cm.getTokenTypeAt(cur)) return CodeMirror.Pass;
var next = cm.getRange(cur, Pos(cur.line, cur.ch + 1));
if (range.empty()) if (left == right && next == right) curType = cm.getRange(cur, Pos(cur.line, cur.ch + 3)) == left + left + left ? "skipThree" :"skip"; else if (left == right && cur.ch > 1 && cm.getRange(Pos(cur.line, cur.ch - 2), cur) == left + left && (cur.ch <= 2 || cm.getRange(Pos(cur.line, cur.ch - 3), Pos(cur.line, cur.ch - 2)) != left)) curType = "addFour"; else {
if (left == right && CodeMirror.isWordChar(next)) return CodeMirror.Pass;
...
this.cm = cm, this.getHints = getHints, this.options = options, this.widget = this.onClose = null;
}
...
Tab:handle.pick,
Esc:handle.close
}, ourMap = options.customKeys ? {} :baseMap;
if (options.customKeys) for (var key in options.customKeys) options.customKeys.hasOwnProperty(key) && addBinding(key, options.customKeys[key]);
if (options.extraKeys) for (var key in options.extraKeys) options.extraKeys.hasOwnProperty(key) && addBinding(key, options.extraKeys[key]);
return ourMap;
}
...
var widget = this, cm = completion.cm, options = completion.options, hints = this.hints = document.createElement("ul");
hints.className = "CodeMirror-hints", this.selectedHint = options.getDefaultSelection ? options.getDefaultSelection(cm, options, data) :0;
...
var pos = cm.cursorCoords(options.alignWithWord !== !1 ? data.from :null), left = pos.left, top = pos.bottom, below = !0;
...
var t = getHintElement(hints, e.target || e.srcElement);
t && null != t.hintId && (widget.changeActive(t.hintId), options.completeOnSingleClick && widget.pick());
...
return CodeMirror.signal(cm, "startCompletion", cm), completion.options.async ? (getHints(cm, function(hints) {
completion.showHints(hints);
}, completion.options), void 0) :completion.showHints(getHints(cm, completion.options));
}
...
completion.hint ? completion.hint(this.cm, data, completion) :this.cm.replaceRange(getText(completion), completion.from || data.from, completion.to || data.to), 
CodeMirror.signal(data, "pick", completion), this.close();
},
showHints:function(data) {
return data && data.list.length && this.active() ? (0 != this.options.completeSingle && 1 == data.list.length ? this.pick(data, 0) :this.showWidget(data), 
void 0) :this.close();
},
...
function update() {
finished || (CodeMirror.signal(data, "update"), completion.options.async ? completion.getHints(completion.cm, finishUpdate, completion.options) :finishUpdate(completion.getHints(completion.cm, completion.options)));
}
function finishUpdate(data_) {
return data = data_, finished ? void 0 :data && data.list.length ? (completion.widget = new Widget(completion, data), 
void 0) :done();
}
...
var finished, debounce = 0, completion = this, closeOn = this.options.closeCharacters || /[\s()\[\]{};:>,]/, startPos = this.cm.getCursor(), startLen = this.cm.getLine(startPos.line).length, requestAnimationFrame = window.requestAnimationFrame || function(fn) {
...
this.completion.options.closeOnUnfocus !== !1 && (cm.off("blur", this.onBlur), cm.off("focus", this.onFocus)), 
