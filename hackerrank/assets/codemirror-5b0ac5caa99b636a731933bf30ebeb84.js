...
} else if ("number" == type || "string" == type) cx.marked = jsonldMode ? "property" :type + " property"; else if ("[" == type) return cont(expression, expect("]"), afterprop);
...
return "prop" == cx.type && "}" == ch && (cx = cx.prev), !cx.prev || ("}" != ch || "block" != cx.type && "top" != cx.type && "interpolation" != cx.type && "font_face" != cx.type) && (")" != ch || "parens" != cx.type && "params" != cx.type && "media_parens" != cx.type) && ("{" != ch || "at" != cx.type && "media" != cx.type) || (indent = cx.indent - indentUnit, 
cx = cx.prev), indent;
},
electricChars:"}",
blockCommentStart:"/*",
...
return external;
}), CodeMirror.defineMIME("text/x-vb", "vb"), CodeMirror.defineMode("cobol", function() {
function makeKeywords(str) {
for (var obj = {}, words = str.split(" "), i = 0; i < words.length; ++i) obj[words[i]] = !0;
return obj;
}
...
}), CodeMirror.defineMIME("text/x-cobol", "cobol"), CodeMirror.defineMode("tcl", function() {
...
}), CodeMirror.defineMIME("text/x-tcl", "tcl"), CodeMirror.defineMode("smalltalk", function(config) {
...
adjustHeight = parseInt(adjustHeight), adjustHeight = adjustHeight - height - (pos.bottom - pos.top), 
hints.style.top = adjustHeight + "px", below = !1;
} else if (height > winH) {
