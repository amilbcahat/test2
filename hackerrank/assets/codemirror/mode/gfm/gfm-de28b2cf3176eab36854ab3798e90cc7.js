...
if (state.codeBlock) return stream.match(/^```/) ? (state.codeBlock = !1, null) :(stream.skipToEnd(), 
null);
...
if (state.ateSpace = !1, stream.match(/^(?:[a-zA-Z0-9\-_]+\/)?(?:[a-zA-Z0-9\-_]+@)?(?:[a-f0-9]{7,40}\b)/)) return "link";
if (stream.match(/^(?:[a-zA-Z0-9\-_]+\/)?(?:[a-zA-Z0-9\-_]+)?#[0-9]+\b/)) return "link";
}
return stream.match(/^((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]|\([^\s()<>]*\))+(?:\([^\s()<>]*\)|[^\s`!()\[\]{};:'".,<>?\xab\xbb\u201c\u201d\u2018\u2019]))/i) && "](" != stream.string.slice(stream.start - 2, stream.start) ? "link" :(stream.next(), 
null);
},
blankLine:blankLine
}, markdownConfig = {
