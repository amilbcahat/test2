...
function f(type) {
return type == wanted ? cont() :";" == wanted ? pass() :cont(f);
}
return f;
}
...
cont()) :"variable" == type || "property" == type || "." == type || "*" == value ? cont(importdef) :void 0;
}
function typedef(type, value) {
return "variable" == type && /[A-Z]/.test(value.charAt(0)) ? (registerimport(value), 
cont()) :"type" == type && /[A-Z]/.test(value.charAt(0)) ? cont() :void 0;
}
...
}), CodeMirror.defineMIME("text/x-haxe", "haxe"), CodeMirror.defineMode("hxml", function() {
return {
startState:function() {
return {
define:!1,
...
token:function(stream, state) {
var ch = stream.peek(), sol = stream.sol();
if ("#" == ch) return stream.skipToEnd(), "comment";
if (sol && "-" == ch) {
var style = "variable-2";
return stream.eat(/-/), "-" == stream.peek() && (stream.eat(/-/), style = "keyword a"), 
"D" == stream.peek() && (stream.eat(/[D]/), style = "keyword c", state.define = !0), 
stream.eatWhile(/[A-Z]/i), style;
}
var ch = stream.peek();
return 0 == state.inString && "'" == ch && (state.inString = !0, ch = stream.next()), 
1 == state.inString ? (stream.skipTo("'") || stream.skipToEnd(), "'" == stream.peek() && (stream.next(), 
state.inString = !1), "string") :(stream.next(), null);
},
lineComment:"#"
};
