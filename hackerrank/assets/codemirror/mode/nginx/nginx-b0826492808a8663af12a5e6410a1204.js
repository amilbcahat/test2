...
if (keywords_block.propertyIsEnumerable(cur)) return "variable-2";
if (keywords_important.propertyIsEnumerable(cur)) return "string-2";
var ch = stream.next();
return "@" == ch ? (stream.eatWhile(/[\w\\\-]/), ret("meta", stream.current())) :"/" == ch && stream.eat("*") ? (state.tokenize = tokenCComment, 
tokenCComment(stream, state)) :"<" == ch && stream.eat("!") ? (state.tokenize = tokenSGMLComment, 
tokenSGMLComment(stream, state)) :"=" != ch ? "~" != ch && "|" != ch || !stream.eat("=") ? '"' == ch || "'" == ch ? (state.tokenize = tokenString(ch), 
state.tokenize(stream, state)) :"#" == ch ? (stream.skipToEnd(), ret("comment", "comment")) :"!" == ch ? (stream.match(/^\s*\w*/), 
ret("keyword", "important")) :/\d/.test(ch) ? (stream.eatWhile(/[\w.%]/), ret("number", "unit")) :/[,.+>*\/]/.test(ch) ? ret(null, "select-op") :/[;{}:\[\]]/.test(ch) ? ret(null, ch) :(stream.eatWhile(/[\w\\\-]/), 
ret("variable", "variable")) :ret(null, "compare") :(ret(null, "compare"), void 0);
}
...
return "hash" == type && "rule" == context ? style = "atom" :"variable" == style && ("rule" == context ? style = "number" :context && "@media{" != context || (style = "tag")), 
"rule" == context && /^[\{\};]$/.test(type) && state.stack.pop(), "{" == type ? "@media" == context ? state.stack[state.stack.length - 1] = "@media{" :state.stack.push("{") :"}" == type ? state.stack.pop() :"@media" == type ? state.stack.push("@media") :"{" == context && "comment" != type && state.stack.push("rule"), 
style;
},
indent:function(state, textAfter) {
var n = state.stack.length;
return /^\}/.test(textAfter) && (n -= "rule" == state.stack[state.stack.length - 1] ? 2 :1), 
