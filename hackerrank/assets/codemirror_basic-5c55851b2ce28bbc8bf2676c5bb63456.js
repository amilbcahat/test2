...
var data;
return that.options.showFileTree ? (that._checkAndSave(), data = {}) :(data = {
...
var data;
return that.options.showFileTree ? that.currentFile && !that.currentFile.original.readOnly ? (that.currentFile.original.fileContents = that.value(), 
data = that.currentFile) :data = {} :(data = {
code:that.codeEditor.getValue(),
language:that.options.language
}, that.options.showCustomInput && $(that.element).find("#customtestcase").attr("checked") && (data.custominput = $(that.element).find("#custominput").val())), 
...
this._autoCompleteReqestStatus = !1, this._supportedLanguages = [ "java", "c", "cpp" ], 
...
var currentValue;
return e = $(_this.element), treeObj || that.currentFile && !that.currentFile.original.readOnly && (currentValue = that.value(), 
that.currentFile.original.fileContents.code !== currentValue.code && (that.currentFile.original.fileContents = currentValue, 
that.options.showFileTree.onSave(that.currentFile))), "file" === treeObj.node.type ? (that._showLoading(), 
...
check_lang ? newTreeObj.node.original.fileContents.language = check_lang :(newTreeObj.node.original.fileContents.code = "Unknown File format, do you want to download this file and check the data.", 
newTreeObj.node.original.readOnly = !0, newTreeObj.node.original.fileContents.language = "text"), 
that.currentFile && newTreeObj.node.id !== e.find("#filetree").jstree("get_selected")[0] ? void 0 :(that.currentFile = newTreeObj.node, 
that.setValue(newTreeObj.node.original.fileContents));
}), treeObj.node.original.readOnly ? that.codeEditor.setOption("readOnly", !0) :that.codeEditor.setOption("readOnly", !1)) :void 0;
};
}(this),
