...
foldCode:!1,
dynamicMode:!1,
languages:[ "c", "cpp", "java", "csharp", "haskell", "php", "python", "perl", "ruby", "bash", "oracle", "mysql", "sql", "clojure", "scala", "code", "text", "brainfuck", "javascript", "d", "go", "lua", "erlang", "sbcl", "ocaml", "pascal", "python3", "groovy", "text_pseudo", "objectivec", "fsharp", "visualbasic", "cobol", "tsql", "lolcode", "smalltalk", "tcl", "html", "css" ],
language:"c",
showTemplate:!0,
...
return $(that.element).find("#custom-input-pane").toggle(this.checked);
...
}(this)), e.find(".code-body").css("height", "400px"), e.find(".code-editors").css("max-height", "400px"), 
...
var retHintValues;
return retHintValues = [], ("Java" === this.options.language || "java" === this.options.language) && (data = data.completions), 
...
var bodyvar, e, footervar, fullselector, fullselector_parent_obj, headervar, opts, pos, sel, that, _ref;
...
}), this.options.fullScreenSelector.length > 0 && (fullselector = $(this.options.fullScreenSelector), 
headervar = null, bodyvar = null, footervar = null, fullselector_parent_obj = null, 
pos = null, that = this, e.find(".fullscreen.active-link.no-select").click(function() {
var lastIndex, separator_width;
return e.append("<div class='codeshell-fullscreen-wrapper'></div>"), e.find(".codeshell-fullscreen-wrapper").append("<div class='codeshell-left-sidebar sidebar'></div>"), 
...
headervar = e.find(".clearfix.grey-header.cap.plL.plR.psT.psB").detach(), bodyvar = e.find(".hr_tour-code-solution").detach(), 
footervar = e.find(".clearfix.mlT").detach(), fullselector_parent_obj = fullselector.parent(), 
pos = fullselector.parent().children().index(fullselector), fullselector.detach(), 
lastIndex = fullselector_parent_obj.children().size(), fullselector_parent_obj.append("<div class='fullscreen-selector-marker'></div>"), 
lastIndex > pos && fullselector_parent_obj.children().eq(pos).before(fullselector_parent_obj.children().last()), 
headervar.appendTo(".codeshell-right-sidebar"), bodyvar.appendTo(".codeshell-right-sidebar"), 
footervar.appendTo(".codeshell-right-sidebar"), fullselector.appendTo(".codeshell-left-sidebar"), 
$("body").addClass("fullscreen-body"), separator_width = 7, e.find(".stretch").click(function() {
return e.find(".stretch").hasClass("go-right") ? (e.find(".codeshell-right-sidebar").css("width", "49.5%"), 
e.find(".codeshell-left-sidebar").css("width", "49.5%"), e.find(".stretch").addClass("go-left"), 
...
var lastIndex;
return headervar.appendTo(e), bodyvar.appendTo(e), footervar.appendTo(e), pos = fullselector_parent_obj.children().index($(".fullscreen-selector-marker")), 
...
e.find(".codeshell-fullscreen-wrapper").detach(), $("body").removeClass("fullscreen-body"), 
e.find(".fullscreen.active-link.no-select.force-hide").removeClass("force-hide"), 
e.find(".restorefullscreen.active-link.no-select").addClass("force-hide"), e.find(".code-body").css("height", "400px"), 
e.find(".code-editors").css("max-height", "400px");
...
var doc_height;
return $("body").hasClass("fullscreen-body") ? (doc_height = $(window).height() - ($(".clearfix.grey-header.cap.plL.plR.psT.psB").height() + $(".clearfix.mlT").height() + 120) + "px", 
$(".code-body").css("height", doc_height), $(".code-editors").css("max-height", doc_height), 
$(".code-editors").css("height", doc_height)) :void 0;
},
