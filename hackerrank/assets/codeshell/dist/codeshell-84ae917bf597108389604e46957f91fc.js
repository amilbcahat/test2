...
}(this)), this.options.enableIntellisense ? e.find("#auto-complete").prop("checked", !0) :e.find("#auto-complete").prop("checked", !1), 
e.find("#auto-complete").click(function(_this) {
return function() {
_this.options.enableIntellisense = e.find("#auto-complete").is(":checked") ? !0 :!1;
};
...
var data, retHintValues, that;
if (this.options.enableIntellisense !== !1) return data = this._getFullCodeOffsetLanguage(cm), 
...
var autoCompleteInfo, code, language, lineCh, lineNum, offset;
for (language = this.options.language, code = "", 0 !== this._getLangHeadCode(language).length && (code += this._getLangHeadCode(language)), 
code += "\n", offset = code.length, lineCh = cm.getCursor(), lineNum = 0; lineNum < lineCh.line; ) offset += cm.getLine(lineNum).length + 1, 
lineNum++;
return offset += lineCh.ch, code += cm.getValue(), code += "\n", 0 !== this._getLangTailCode(language).length && (code += this._getLangTailCode(language)), 
code += "\n", autoCompleteInfo = {
code:code,
offset:offset,
fileType:language
