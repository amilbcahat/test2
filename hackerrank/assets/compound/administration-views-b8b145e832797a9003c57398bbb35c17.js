...
return function() {
return _this.render_input_editor(), _this.render_output_editor();
};
}(this)), this.test_case && (void 0 !== (_ref = this.test_case.get("tag")) && null !== _ref && $("input#tag").val(this.test_case.get("tag")), 
...
this.test_case.get("additional") === !0 ? $("input#additional").prop("checked", !0) :$("input#additional").prop("checked", !1)), 
...
if ($input_textarea = this.$("#input_textarea").get(0), $input_textarea && !this.input_editor && (this.input_editor = CodeMirror.fromTextArea($input_textarea, this.editorOptions), 
...
if ($output_textarea = this.$("#output_textarea").get(0), $output_textarea && !this.output_editor && (this.output_editor = CodeMirror.fromTextArea($output_textarea, this.editorOptions), 
...
var additional, cont, data, errors, errr, form_data, highs, http_method, input_type, input_value, lows, output_type, output_value, sample, strength, tag, that, url_suffix;
...
additional = $("input#additional").is(":checked"), this.test_case_index && this.test_case.get("additional") === additional || (data.additional = additional), 
...
jQuery(function() {
var Administration_ChallengeEditCustomCheckerView, HR, _ref;
return Administration_ChallengeEditCustomCheckerView = function(_super) {
function Administration_ChallengeEditCustomCheckerView() {
return Administration_ChallengeEditCustomCheckerView.__super__.constructor.apply(this, arguments);
}
return __extends(Administration_ChallengeEditCustomCheckerView, _super), Administration_ChallengeEditCustomCheckerView.prototype.template = "administration/challenge-edit-custom-checker", 
Administration_ChallengeEditCustomCheckerView.prototype.className = "administration-challenge-edit-custom-checker", 
Administration_ChallengeEditCustomCheckerView.prototype.initialize = function(options) {
return null == options && (options = {}), this.listenTo(this.model, "reset", this.render);
}, Administration_ChallengeEditCustomCheckerView.prototype.events = {
"codeshellsubmit #codeeditor":"saveCustomLangCode"
}, Administration_ChallengeEditCustomCheckerView.prototype.render = function() {
var that;
...
increaseArea:"20%"
}), that = this, this.$("input[type=checkbox]").on("ifChecked", function(e) {
var el;
return el = $(e.currentTarget).attr("id"), that.saveIt(el, !0);
}), this.$("input[type=checkbox]").on("ifUnchecked", function(e) {
var el;
return el = $(e.currentTarget).attr("id"), that.saveIt(el, !1);
}), HR.appController.loadCodeMirror(function(_this) {
return function() {
return _this.$("#codeeditor").codeshell({
showCustomInput:!1,
showCompileTest:!1,
showSubmit:!0,
...
}(this)), _.isString(this.model.get("checker_program")) && _.isString(this.model.get("custom_language")) ? HR.appController.loadCodeMirror(function(_this) {
return function() {
return _this.$("#codeeditor").codeshell("setValue", {
code:_this.model.get("checker_program"),
language:_this.model.get("custom_language")
});
};
...
}, Administration_ChallengeEditCustomCheckerView.prototype.saveCustomLangCode = function(e, data) {
var $btn;
return $btn = $(e.currentTarget).find("button.bb-submit"), "disabled" !== $btn.attr("disabled") ? ($btn.attr("disabled", "disabled"), 
this.model.set("checker_program", data.code), this.model.set("custom_language", data.language), 
$btn.html("Saving..."), this.model.save(null, {
silent:!0,
success:function(model, resp) {
return $btn.removeAttr("disabled"), resp.status ? ($btn.html("Saved"), setTimeout(function() {
return $btn.html("Save");
}, 2e3)) :($btn.html("Save"), alert("Error Occured"));
}
})) :void 0;
}, Administration_ChallengeEditCustomCheckerView.prototype.saveIt = function(el, value) {
var that;
return that = this, this.model.set(el, value), this.$("div.state." + el).html("saving...").show(), 
this.model.save(null, {
success:function(model, resp) {
return resp.status ? (that.$("div.state." + el).html("saved").show(), setTimeout(function() {
return that.$("div.state." + el).html("").hide();
}, 2e3)) :that.$("div.state." + el).html("error occured").show();
}
});
}, Administration_ChallengeEditCustomCheckerView;
}(window.HR.GenericView), HR = null != (_ref = window.HR) ? _ref :{}, HR.Administration_ChallengeEditCustomCheckerView = Administration_ChallengeEditCustomCheckerView;
});
}.call(this), function() {
var __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
function ctor() {
...
jQuery(function() {
var Administration_ChallengeEditEditorialView, HR, _ref;
return Administration_ChallengeEditEditorialView = function(_super) {
function Administration_ChallengeEditEditorialView() {
return Administration_ChallengeEditEditorialView.__super__.constructor.apply(this, arguments);
}
return __extends(Administration_ChallengeEditEditorialView, _super), Administration_ChallengeEditEditorialView.prototype.template = "administration/challenge-edit-editorial", 
Administration_ChallengeEditEditorialView.prototype.className = "administration-challenge-edit-editorial", 
Administration_ChallengeEditEditorialView.prototype.initialize = function(options) {
...
}, Administration_ChallengeEditEditorialView.prototype.events = {
"focusout input.auto-save":"autoSave"
}, Administration_ChallengeEditEditorialView.prototype.render = function() {
return HR.requires([ "MathJax/MathJax", "sanitize" ], function(_this) {
return function() {
var that;
return $(_this.el).html(HR.appController.template(_this.template, _this)({
model:_this.model,
_model:_this.model.toJSON(),
action:_this.action
})), that = _this, HR.appController.loadCodeMirrorMode("markdown", function() {
return _this.applyCodeMirror();
});
};
}(this)), this;
}, Administration_ChallengeEditEditorialView.prototype.applyCodeMirror = function() {
...
tabSize:4,
lineWrapping:!0
}, that = this, this.$("textarea.approach").get(0) && !this.approach_editor && (this.approach_editor = CodeMirror.fromTextArea(this.$("textarea.approach").get(0), editorOptions), 
this.approach_editor.on("change", this.throttledSave), this.approach_editor.on("change", function(_this) {
return function() {
return that.updatePreview(_this.approach_editor, ".preview-wrap-approach", "#approach-preview");
};
}(this))), this.$("textarea.setter_code").get(0) && !this.setter_code_editor && (this.setter_code_editor = CodeMirror.fromTextArea(this.$("textarea.setter_code").get(0), editorOptions), 
this.setter_code_editor.on("change", this.throttledSave), this.setter_code_editor.on("change", function(_this) {
return function() {
return that.updatePreview(_this.setter_code_editor, ".preview-wrap-setter_code", "#setter_code-preview");
};
}(this))), this.$("textarea.tester_code").get(0) && !this.tester_code_editor && (this.tester_code_editor = CodeMirror.fromTextArea(this.$("textarea.tester_code").get(0), editorOptions), 
this.tester_code_editor.on("change", this.throttledSave), this.tester_code_editor.on("change", function(_this) {
return function() {
return that.updatePreview(_this.tester_code_editor, ".preview-wrap-tester_code", "#tester_code-preview");
};
}(this))), this.setData(this.model.toJSON());
}, Administration_ChallengeEditEditorialView.prototype.setData = function(data) {
var ex_data;
return ex_data = data.editorial._data, ex_data && (ex_data.difficulty && this.$("input#difficulty").val(ex_data.difficulty), 
ex_data.required_knowledge && this.$("input#required_knowledge").val(ex_data.required_knowledge), 
ex_data.time_complexity && this.$("input#time_complexity").val(ex_data.time_complexity), 
ex_data.editorialist && this.$("input#editorialist").val(ex_data.editorialist), 
ex_data.setter_name && this.$("input#setter_name").val(ex_data.setter_name), ex_data.tester_name && this.$("input#tester_name").val(ex_data.tester_name), 
ex_data.approach_markdown && this.approach_editor && this.approach_editor.setValue(ex_data.approach_markdown), 
ex_data.setter_code_markdown && this.setter_code_editor && this.setter_code_editor.setValue(ex_data.setter_code_markdown), 
ex_data.tester_code_markdown && this.tester_code_editor && this.tester_code_editor.setValue(ex_data.tester_code_markdown)), 
this.checkRenderedData();
}, Administration_ChallengeEditEditorialView.prototype.checkRenderedData = function() {
return this.rendered() && !this.changed() ? this.data_rendered = !0 :void 0;
}, Administration_ChallengeEditEditorialView.prototype.rendered = function() {
return this.$("textarea.approach").length > 0 && this.$("textarea.setter_code").length > 0 && this.$("textarea.tester_code").length > 0 && this.approach_editor && this.setter_code_editor && this.tester_code_editor ? !0 :!1;
}, Administration_ChallengeEditEditorialView.prototype.changed = function() {
var ex_data;
return ex_data = this.model.get("editorial")._data, ex_data ? ex_data.difficulty !== this.$("input#difficulty").val() ? !0 :ex_data.required_knowledge !== this.$("input#required_knowledge").val() ? !0 :ex_data.time_complexity !== this.$("input#time_complexity").val() ? !0 :ex_data.approach_markdown !== this.approach_editor.getValue() ? !0 :ex_data.setter_code_markdown !== this.setter_code_editor.getValue() ? !0 :ex_data.tester_code_markdown !== this.tester_code_editor.getValue() ? !0 :!1 :!1;
}, Administration_ChallengeEditEditorialView.prototype.validate = function() {
return !0;
}, Administration_ChallengeEditEditorialView.prototype.getData = function() {
var data;
return data = {}, data.approach = this.approach_editor ? this.approach_editor.getValue() :null, 
data.setter_code = this.setter_code_editor ? this.setter_code_editor.getValue() :null, 
data.tester_code = this.tester_code_editor ? this.tester_code_editor.getValue() :null, 
data.difficulty = this.$("input#difficulty").val(), data.required_knowledge = this.$("input#required_knowledge").val(), 
data.time_complexity = this.$("input#time_complexity").val(), data.editorialist = this.$("input#editorialist").val(), 
data.setter_name = this.$("input#setter_name").val(), data.tester_name = this.$("input#tester_name").val(), 
data.setter_code_markdown = data.setter_code, data.tester_code_markdown = data.tester_code, 
data.approach_markdown = data.approach, data;
}, Administration_ChallengeEditEditorialView.prototype.autoSave = function() {
return this.saveChallenge();
}, Administration_ChallengeEditEditorialView.prototype.updatePreview = function(e, preview_wrap, preview) {
return HR.requires([ "MathJax/MathJax", "sanitize" ], function(_this) {
return function() {
var sanitized_text;
return sanitized_text = HackDown.processData(e.getValue()), "" === sanitized_text ? _this.$(preview_wrap).hide() :_this.$(preview_wrap).show(), 
_this.$(preview).html(sanitized_text);
};
}(this));
}, Administration_ChallengeEditEditorialView;
}(window.HR.GenericView), HR = null != (_ref = window.HR) ? _ref :{}, HR.Administration_ChallengeEditEditorialView = Administration_ChallengeEditEditorialView;
});
}.call(this), function() {
var __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
function ctor() {
...
jQuery(function() {
var Administration_ChallengeEditFlagsView, HR, _ref;
return Administration_ChallengeEditFlagsView = function(_super) {
function Administration_ChallengeEditFlagsView() {
return Administration_ChallengeEditFlagsView.__super__.constructor.apply(this, arguments);
}
return __extends(Administration_ChallengeEditFlagsView, _super), Administration_ChallengeEditFlagsView.prototype.template = "administration/challenge-edit-flags", 
Administration_ChallengeEditFlagsView.prototype.className = "administration-challenge-edit-flags", 
Administration_ChallengeEditFlagsView.prototype.initialize = function(options) {
return null == options && (options = {}), this.listenTo(this.model, "reset", this.render), 
this.listenTo(this.model, "sync", this.render);
}, Administration_ChallengeEditFlagsView.prototype.render = function() {
var that;
return $(this.el).html(HR.appController.template(this.template, this)({
model:this.model,
_model:this.model.toJSON()
...
var el;
return el = $(e.currentTarget).attr("id"), that.saveIt(el, !1);
}), this.$("#solved_score").val(this.model.get("solved_score"));
}, Administration_ChallengeEditFlagsView.prototype.validate = function() {
return !0;
}, Administration_ChallengeEditFlagsView.prototype.getData = function() {
return {
solved_score:this.$("#solved_score").val()
};
}, Administration_ChallengeEditFlagsView.prototype.saveIt = function(el, value) {
var that;
return that = this, this.model.set(el, value), this.$("div.state." + el).html("saving...").show(), 
this.model.save(null, {
success:function(model, resp) {
...
}, Administration_ChallengeEditFlagsView;
}(window.HR.GenericView), HR = null != (_ref = window.HR) ? _ref :{}, HR.Administration_ChallengeEditFlagsView = Administration_ChallengeEditFlagsView;
});
}.call(this), function() {
var __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
function ctor() {
...
}, HR.appController.loadCodeMirrorMode(this.language_key, function(_this) {
return function() {
var $tbody_textarea, $thead_textarea, $ttail_textarea;
return $thead_textarea = _this.$("textarea.template_head").get(0), $thead_textarea && !_this.thead_editor && (_this.thead_editor = CodeMirror.fromTextArea($thead_textarea, _this.editorOptions), 
_this.thead_editor.setValue(_this.model.get("" + _this.language_key + "_template_head") || lang_default_text[_this.language_key] || ""), 
_this.thead_editor.focus()), $tbody_textarea = _this.$("textarea.template_body").get(0), 
$tbody_textarea && !_this.tbody_editor && (_this.tbody_editor = CodeMirror.fromTextArea($tbody_textarea, _this.editorOptions), 
_this.tbody_editor.setValue(_this.model.get("" + _this.language_key + "_template") || _this.model.get("" + _this.language_key + "_template_body") || "")), 
$ttail_textarea = _this.$("textarea.template_tail").get(0), $ttail_textarea && !_this.ttail_editor && (_this.ttail_editor = CodeMirror.fromTextArea($ttail_textarea, _this.editorOptions), 
_this.ttail_editor.setValue(_this.model.get("" + _this.language_key + "_template_tail") || "")), 
that = _this, setTimeout(function() {
...
"change input#translation_language":"changeLanguage",
"change input#challenge_difficulty":"changeDifficulty"
...
_model:this.model.toJSON(),
action:this.action
})), this.setupTranslationLanguageSelect(), this.setupChallengeDifficulty(), that = this, 
this.$("input.tags").length > 0 && this.$("input.tags").tagsInput({
width:"98%",
...
}), HR.appController.loadCodeMirrorMode("markdown", function(_this) {
return function() {
return _this.applyCodeMirror();
};
}(this)), this;
}, Administration_ChallengeEditOverviewView.prototype.applyCodeMirror = function() {
var editorOptions, that;
return editorOptions = {
lineNumbers:!0,
lineWrapping:!0,
...
tabSize:4,
lineWrapping:!0
}, that = this, this.$("textarea.problem-statement").get(0) && !this.problem_statement_editor && (this.problem_statement_editor = CodeMirror.fromTextArea(this.$("textarea.problem-statement").get(0), editorOptions), 
this.updatePreview(), this.problem_statement_editor.on("change", this.throttledSave), 
this.problem_statement_editor.on("change", function() {
return that.updatePreview();
})), this.setData(this.model.toJSON());
}, Administration_ChallengeEditOverviewView.prototype.setData = function(data) {
var current_language, data_clone, filter;
if ("create" !== this.action) return current_language = $("input#translation_language").val(), 
"English" !== current_language ? (filter = _.select(this.model.get("available_translations"), function(translation) {
return translation.language === current_language;
}), data_clone = 1 === filter.length ? filter[0] :{}) :data_clone = _.clone(data), 
data_clone.name ? this.$("input#name").val(data_clone.name) :this.$("input#name").val(""), 
data_clone.preview ? (this.$("textarea.description").val(data_clone.preview), data_clone.preview.length > this.model.max_limit.description ? this.$("small.description").html("Characters left: -" + (data_clone.preview.length - this.model.max_limit.description)).addClass("error").removeClass("hint") :this.$("small.description").html("Characters left: " + (this.model.max_limit.description - data_clone.preview.length)).addClass("hint").removeClass("error")) :(this.$("textarea.description").val(""), 
this.$("small.description").html("Characters left: " + this.model.max_limit.description).addClass("hint").removeClass("error")), 
data_clone.body && this.problem_statement_editor ? this.problem_statement_editor.setValue(data_clone.body) :this.problem_statement_editor.setValue(""), 
...
var current_language, filter, filters;
if (current_language = $("input#translation_language").val(), "English" !== current_language) {
if (filters = _.select(this.model.get("available_translations"), function(translation) {
return translation.language === current_language;
}), 1 === filters.length) {
if (filter = filters[0], filter.name !== this.$("input#name").val()) return !0;
if (filter.preview !== this.$("textarea.description").val()) return !0;
if (filter.body !== this.problem_statement_editor.getValue()) return !0;
}
} else {
if (this.model.get("name") !== this.$("input#name").val()) return !0;
...
return _.difference(this.model.getTags(), this.getTags()).length > 0 || _.difference(this.getTags(), this.model.getTags()).length > 0 ? !0 :this.model.get("difficulty") !== this.getDifficulty() && this.difficultyChanged ? !0 :!1;
...
var data;
return data = {}, data.translation_language = this.$("input#translation_language").val(), 
data.name = this.$("input#name").val(), data.preview = this.$("textarea.description").val(), 
"" === data.preview && (data.preview = null), data.body = this.problem_statement_editor ? this.problem_statement_editor.getValue() :null, 
data.tags = this.getTags(), "" === data.body && (data.body = null), data.difficulty = this.getDifficulty(), 
data;
...
}, Administration_ChallengeEditOverviewView.prototype.getDifficulty = function() {
return this.difficultyChanged ? this.difficulty_data[this.$("#challenge_difficulty").val() || 2].value :this.model.get("difficulty");
...
}, Administration_ChallengeEditOverviewView.prototype.updatePreview = function() {
return HR.requires([ "MathJax/MathJax", "sanitize" ], function(_this) {
return function() {
var sanitized_text;
return sanitized_text = HackDown.processData(_this.problem_statement_editor.getValue()), 
"" === sanitized_text ? _this.$(".preview-wrap").hide() :_this.$(".preview-wrap").show(), 
_this.$("#problem-statement-preview").html(sanitized_text);
};
}(this));
}, Administration_ChallengeEditOverviewView.prototype.setupTranslationLanguageSelect = function() {
var data, languages;
return languages = HR.TRANSLATION_LANGUAGES, data = [], _.each(languages, function(_this) {
return function(code, name) {
return data.push({
id:name,
text:name
}), $(_this.el).find("#translation_language").select2({
data:data,
width:"copy"
}), _this.setLocalLanguage(_this.model.get("local_language") || "English");
};
}(this));
}, Administration_ChallengeEditOverviewView.prototype.setLocalLanguage = function(local_language) {
return $(this.el).find("#translation_language").val(local_language), $(this.el).find("#s2id_translation_language .select2-choice span").text(local_language);
}, Administration_ChallengeEditOverviewView.prototype.changeLanguage = function() {
return this.setData(this.model.toJSON());
}, Administration_ChallengeEditOverviewView.prototype.changeDifficulty = function() {
return this.difficultyChanged = !0;
}, Administration_ChallengeEditOverviewView.prototype.setupChallengeDifficulty = function() {
var difficulty, difficulty_id, i, _i;
for (this.difficulty_data = [ {
id:0,
value:.9,
min:.8,
text:"Easy"
...
min:0,
text:"Expert"
} ], difficulty = this.model.get("difficulty"), difficulty_id = 4, i = _i = 0; 4 >= _i; i = ++_i) if (difficulty >= this.difficulty_data[i].min) {
difficulty_id = i;
break;
}
return $("#challenge_difficulty").select2({
minimumResultsForSearch:-1,
data:this.difficulty_data
}).select2("val", "" + difficulty_id);
...
model:this.model
}), this.add_subview(this.tab_view)) :"custom-checker" === this.tab ? (this.tab_view = new HR.Administration_ChallengeEditCustomCheckerView({
model:this.model
}), this.add_subview(this.tab_view)) :"flags" === this.tab ? (this.tab_view = new HR.Administration_ChallengeEditFlagsView({
model:this.model
}), this.add_subview(this.tab_view)) :"testcases" === this.tab ? (this.tab_view = new HR.Administration_ChallengeEditTestcasesView({
model:this.model
}), this.add_subview(this.tab_view)) :"editorial" === this.tab ? (this.tab_view = new HR.Administration_ChallengeEditEditorialView({
model:this.model,
action:this.action,
parent:this
...
HR.appController.loadCodeMirror(function() {
return function() {
...
jQuery(function() {
var Administration_ContestEditAcmPrefsView, HR, _ref;
return Administration_ContestEditAcmPrefsView = function(_super) {
function Administration_ContestEditAcmPrefsView() {
return Administration_ContestEditAcmPrefsView.__super__.constructor.apply(this, arguments);
}
return __extends(Administration_ContestEditAcmPrefsView, _super), Administration_ContestEditAcmPrefsView.prototype.template = "administration/contest-edit-acm-prefs", 
Administration_ContestEditAcmPrefsView.prototype.className = "administration-contest-edit-acm-prefs", 
Administration_ContestEditAcmPrefsView.prototype.initialize = function() {
return this.listenTo(this.model, "reset", this.render);
}, Administration_ContestEditAcmPrefsView.prototype.events = {
"focusout input[type=text]":"saveField"
}, Administration_ContestEditAcmPrefsView.prototype.render = function() {
var that;
return $(this.el).html(HR.appController.template(this.template, this)({
model:this.model,
_model:this.model.toJSON()
...
}, Administration_ContestEditAcmPrefsView.prototype.saveField = function(e) {
var el, value;
return el = $(e.currentTarget).attr("id"), value = $(e.currentTarget).val(), this.saveIt(el, value);
}, Administration_ContestEditAcmPrefsView.prototype.saveIt = function(el, value) {
var that;
return that = this, this.model.set(el, value), this.$("div.state." + el).html("saving...").show(), 
this.model.save(null, {
success:function(model, resp) {
...
}, Administration_ContestEditAcmPrefsView;
}(window.HR.GenericView), HR = null != (_ref = window.HR) ? _ref :{}, HR.Administration_ContestEditAcmPrefsView = Administration_ContestEditAcmPrefsView;
});
}.call(this), function() {
var __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
function ctor() {
...
"click button.moderator-save":"addMod",
"click a.remove-moderator":"removeMod",
"click input#college-checkbox":"disableCollege"
...
tabSize:4,
lineWrapping:!0
}, that = this, HR.appController.loadCodeMirrorMode("markdown", function(_this) {
return function() {
return _this.$("textarea.description").get(0) && !_this.description_editor ? (_this.description_editor = CodeMirror.fromTextArea(_this.$("textarea.description").get(0), editorOptions), 
_this.updatePreview(), _this.description_editor.on("change", _this.throttledSave), 
_this.setData(_this.model.toJSON()), _this.description_editor.on("change", function() {
return that.updatePreview();
})) :void 0;
};
}(this)), "create" !== this.action && this.$("#moderator").length > 0 && (that = this, 
this.$("#moderator").autocomplete({
minLength:2,
source:function(request, response) {
return $.ajax({
...
}), this.setData(this.model.toJSON()), this.setUpEvents(), this.setupCollegeSelect();
...
}, Administration_ContestEditOverviewView.prototype.setCollege = function(college) {
return this.$(".error.college").html("").hide(), college ? ($(this.el).find("#college").val(college), 
$(this.el).find("#s2id_college .select2-choice span").text(college), this.$("input#college-checkbox").attr("checked", !0)) :($(this.el).find("#college").val(""), 
$(this.el).find("#s2id_college .select2-choice span").text(""), this.$("input#college-checkbox").attr("checked", !1), 
this.$("input#college").select2("enable", !1), this.$("#s2id_college").addClass("disabled"));
}, Administration_ContestEditOverviewView.prototype.setupCollegeSelect = function() {
return this.$("input#college").select2(HR.util.school_select2_options), this.setCollege(this.model.get("college"));
}, Administration_ContestEditOverviewView.prototype.disableCollege = function(e) {
return $(e.currentTarget).is(":checked") ? ($("input#college").select2("enable", !0), 
this.$("#s2id_college").removeClass("disabled")) :this.setCollege("");
...
response;
}, Administration_ContestEditOverviewView.prototype.validate_college = function() {
return $("input#college-checkbox").is(":checked") && "" === this.$("input#college").val() ? !1 :!0;
}, Administration_ContestEditOverviewView.prototype.validate = function() {
var timestamps, _enderrors, _starterrors;
return this.$(".error").html("").hide(), timestamps = this.getTimeStamps(), 0 === _.size(timestamps.errors) && "" !== this.$("input#name").val() && this.validate_college() && this.$("textarea.tagline").val().length <= this.model.max_limit.tagline ? !0 :((timestamps.errors.starttime || timestamps.errors.startdate) && (_starterrors = [], 
...
this.validate_college() || this.$(".error.college").html(HR.util.getErrorList([ "College name can't be empty" ])).show(), 
!1);
}, Administration_ContestEditOverviewView.prototype.changed = function() {
var timestamps;
return timestamps = this.getTimeStamps(), this.model.get("starttime") !== timestamps._starttime ? !0 :this.model.get("endtime") !== timestamps._endtime ? !0 :this.$("input#name").val() !== this.model.get("name") ? !0 :this.$("textarea.tagline").val() !== this.model.get("tagline") ? !0 :this.description_editor.getValue() !== this.model.get("description") ? !0 :this.$("input#college").val() !== this.model.get("college") ? !0 :!1;
...
"" === data.description && (data.description = null), data.college = this.$("input#college").val(), 
data;
...
}, Administration_ContestEditOverviewView.prototype.updatePreview = function() {
return HR.requires([ "sanitize" ], function(_this) {
return function() {
var sanitized_text;
return sanitized_text = HackDown.processData(_this.description_editor.getValue()), 
"" === sanitized_text ? _this.$(".preview-wrap").hide() :_this.$(".preview-wrap").show(), 
_this.$("#description-preview").html(sanitized_text);
};
}(this));
...
this.prev_tab && this.tab === this.prev_tab || (this.prev_tab && (this.tab_view.destroy(), 
this._subviews = [], this.$(".tab-body").html("<div class='tabs-body-inner'></div>")), 
"acm-prefs" === this.tab ? (this.tab_view = new HR.Administration_ContestEditAcmPrefsView({
model:this.model,
parent:this
...
page:this.page,
parent:this
}), this.add_subview(this.tab_view)) :"notification" === this.tab ? (notification_collection = HR.collection("broadcast", {
contest_id:this.model.id
}).cached(), this.tab_view = new HR.Administration_ContestNotificationView({
collection:notification_collection,
contest_model:this.model,
contest_id:this.contest_id,
parent:this
...
jQuery(function() {
var Administration_ContestNotificationMailView, HR, _ref;
return Administration_ContestNotificationMailView = function(_super) {
function Administration_ContestNotificationMailView() {
return Administration_ContestNotificationMailView.__super__.constructor.apply(this, arguments);
}
return __extends(Administration_ContestNotificationMailView, _super), Administration_ContestNotificationMailView.prototype.template = "administration/contest-notification-mail", 
Administration_ContestNotificationMailView.prototype.className = "administration-contest-notification-mail", 
Administration_ContestNotificationMailView.prototype.initialize = function(options) {
return this.parent = options.parent, this.listenTo(this.model, "reset", this.render), 
this.render();
}, Administration_ContestNotificationMailView.prototype.render = function() {
return $(this.el).html(HR.appController.template(this.template, this)({
model:this.model,
_model:this.model.toJSON()
}));
}, Administration_ContestNotificationMailView;
}(window.HR.GenericView), HR = null != (_ref = window.HR) ? _ref :{}, HR.Administration_ContestNotificationMailView = Administration_ContestNotificationMailView;
});
}.call(this), function() {
var __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
function ctor() {
...
jQuery(function() {
var Administration_ContestNotificationView, HR, _ref;
return Administration_ContestNotificationView = function(_super) {
function Administration_ContestNotificationView() {
return Administration_ContestNotificationView.__super__.constructor.apply(this, arguments);
}
return __extends(Administration_ContestNotificationView, _super), Administration_ContestNotificationView.prototype.template = "administration/contest-notification", 
Administration_ContestNotificationView.prototype.className = "administration-contest-notification", 
Administration_ContestNotificationView.prototype.initialize = function(options) {
var that;
return this.parent = options.parent, this.contest_model = options.contest_model, 
this.challenge_collection = new HR.Administration_ChallengesCollection(), this.challenge_collection.setContestId(this.contest_model.id), 
that = this, this.challenge_collection.fetch({
success:function() {
var model, model_data, _i, _len, _ref;
for (model_data = [], _ref = that.challenge_collection.models, _i = 0, _len = _ref.length; _len > _i; _i++) model = _ref[_i], 
model_data.push({
id:model.id,
text:model.get("name")
});
return $(".send-general-mail-challenge-list").select2({
data:model_data
});
}
}), this.listenTo(this.collection, "reset", this.render), this.listenTo(this.contest_model, "reset", this.render);
}, Administration_ContestNotificationView.prototype.events = {
"click .info-mail-preview":"showPreview",
"click .send-info-mail-btn":"sendInfoMail",
"click .reminder-mail-preview":"showReminderPreview",
"click .send-reminder-mail-btn":"sendReminderMail",
"change .send-general-mail-type":"changeGeneralMailType",
"click .send-general-mail-btn":"sendGeneralMail"
}, Administration_ContestNotificationView.prototype.changeGeneralMailType = function() {
var value;
return value = $(".send-general-mail-type").val(), "contest-update" === value ? $(".send-general-mail-challenge-list").addClass("hidden") :$(".send-general-mail-challenge-list").removeClass("hidden");
}, Administration_ContestNotificationView.prototype.sendInfoMail = function() {
var id, infoMailModel, that;
return id = parseInt(this.$(".mail-template-id").val().split("/").slice(-1)[0]), 
_.isNumber(id) || (id = 1), infoMailModel = new HR.BroadcastModel({
body:null,
category:"contest-new",
challenge_id:null,
contest_id:this.contest_model.id,
from:HR.profile().get("email"),
hacker_id:HR.profile().id,
using_mail_template:!0,
mail_template_id:id,
subject:null,
status:0
}), that = this, this.$(".send-info-mail-throbber").show(), this.$(".send-info-mail-btn").addClass("disabled"), 
this.$(".send-info-mail-alert").hide(), infoMailModel.save(infoMailModel.attributes, {
error:function(model, response) {
var error_html, error_message, error_messages, _i, _len;
for (error_messages = JSON.parse(response.responseText).errors, error_html = "", 
_i = 0, _len = error_messages.length; _len > _i; _i++) error_message = error_messages[_i], 
error_html = "" + error_html + error_message.field + "<br />";
return that.$(".send-info-mail-throbber").hide(), that.$(".send-info-mail-btn").removeClass("disabled"), 
that.$(".send-info-mail-alert").html(error_html), that.$(".send-info-mail-alert").show();
},
success:function() {
return that.$(".send-info-mail-throbber").hide(), that.$(".send-info-mail-btn").removeClass("disabled");
}
});
}, Administration_ContestNotificationView.prototype.sendGeneralMail = function() {
var challenge_id, generalMailModel, that;
if (generalMailModel = new HR.BroadcastModel({
body:$("#general-broadcast-message").val(),
category:$(".send-general-mail-type").val(),
contest_id:this.contest_model.id,
from:HR.profile().get("email"),
hacker_id:HR.profile().id,
using_mail_template:!1,
subject:$("#general-broadcast-subject").val(),
status:0
}), "challenge-submitted" === generalMailModel.get("category") || "challenge-submitted" === generalMailModel.get("category")) {
if (challenge_id = parseInt($(".send-general-mail-challenge-list").select2("val")), 
!challenge_id) return alert("Challenge has to be selected"), void 0;
generalMailModel.set("challenge_id", challenge_id);
}
return that = this, this.$(".send-general-mail-throbber").show(), this.$(".send-general-mail-btn").addClass("disabled"), 
this.$(".send-general-mail-alert").hide(), generalMailModel.save(generalMailModel.attributes, {
error:function(model, response) {
var error_html, error_message, error_messages, _i, _len;
for (error_messages = JSON.parse(response.responseText).errors, error_html = "", 
_i = 0, _len = error_messages.length; _len > _i; _i++) error_message = error_messages[_i], 
error_html = "" + error_html + error_message.field + "<br />";
return that.$(".send-general-mail-throbber").hide(), that.$(".send-general-mail-btn").removeClass("disabled"), 
that.$(".send-general-mail-alert").html(error_html), that.$(".send-general-mail-alert").show();
},
success:function() {
return that.$(".send-general-mail-throbber").hide(), that.$(".send-general-mail-btn").removeClass("disabled");
}
});
}, Administration_ContestNotificationView.prototype.sendReminderMail = function() {
var id, reminderMailModel, that;
return id = parseInt(this.$(".reminder-mail-template-id").val().split("/").slice(-1)[0]), 
_.isNumber(id) || (id = 2), reminderMailModel = new HR.BroadcastModel({
body:null,
...
subject:null,
status:0
}), that = this, this.$(".send-reminder-mail-throbber").show(), this.$(".send-reminder-mail-btn").addClass("disabled"), 
this.$(".send-reminder-mail-alert").hide(), reminderMailModel.save(reminderMailModel.attributes, {
error:function(model, response) {
var error_html, error_message, error_messages, _i, _len;
for (error_messages = JSON.parse(response.responseText).errors, error_html = "", 
_i = 0, _len = error_messages.length; _len > _i; _i++) error_message = error_messages[_i], 
error_html = "" + error_html + error_message.field + "<br />";
return that.$(".send-reminder-mail-throbber").hide(), that.$(".send-reminder-mail-btn").removeClass("disabled"), 
that.$(".send-reminder-mail-alert").html(error_html), that.$(".send-reminder-mail-alert").show();
},
success:function() {
return that.$(".send-reminder-mail-throbber").hide(), that.$(".send-reminder-mail-btn").removeClass("disabled");
}
});
}, Administration_ContestNotificationView.prototype.showPreview = function() {
var id, mailTemplate, mailTemplateModel;
return "Show Preview" === this.$(".info-mail-preview").html() ? (id = parseInt(this.$(".mail-template-id").val().split("/").slice(-1)[0]), 
_.isNumber(id) || (id = 1), mailTemplateModel = HR.model("mail-template", {
id:id,
sample:!0,
contest_id:this.contest_model.id
}).cached(), mailTemplate = new HR.Administration_ContestNotificationMailView({
model:mailTemplateModel
}), this.assign({
".preview-mail-container":mailTemplate
}), this.$(".preview-mail-container").slideToggle(), this.$(".info-mail-preview").html("Hide Preview")) :(this.$(".preview-mail-container").slideToggle(), 
this.$(".info-mail-preview").html("Show Preview"));
}, Administration_ContestNotificationView.prototype.showReminderPreview = function() {
var id, mailTemplate, mailTemplateModel;
return "Show Preview" === this.$(".reminder-mail-preview").html() ? (id = parseInt(this.$(".reminder-mail-template-id").val().split("/").slice(-1)[0]), 
_.isNumber(id) || (id = 2), mailTemplateModel = HR.model("mail-template", {
id:id,
sample:!0,
contest_id:this.contest_model.id
}).cached(), mailTemplate = new HR.Administration_ContestNotificationMailView({
...
".reminder-mail-container":mailTemplate
}), this.$(".reminder-mail-container").slideToggle(), this.$(".reminder-mail-preview").html("Hide Preview")) :(this.$(".reminder-mail-container").slideToggle(), 
this.$(".reminder-mail-preview").html("Show Preview"));
}, Administration_ContestNotificationView.prototype.render = function() {
return $(this.el).html(HR.appController.template(this.template, this)({
collection:this.collection,
_collection:this.collection.toJSON(),
contest_model:this.contest_model,
_contest_model:this.contest_model.toJSON(),
action:this.action,
hacker:HR.profile().toJSON()
}));
}, Administration_ContestNotificationView;
}(window.HR.GenericView), HR = null != (_ref = window.HR) ? _ref :{}, HR.Administration_ContestNotificationView = Administration_ContestNotificationView;
});
}.call(this), function() {
var __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
function ctor() {
