...
}), register_string = "<p class='margin-large top text-center register'><a href= " + event.url + " class='btn btn-green' target='_blank'>Register</a></p>", 
this.contest.fetch({
success:function(_this) {
return function() {
return _this.contest.get("signed_up") ? ($(".register_div." + event.id).empty(), 
$(".register_div." + event.id).prepend("<p class='margin-large top text-center'>Registered</p>")) :($(".register_div." + event.id).empty(), 
$(".register_div." + event.id).prepend(register_string));
};
}(this)
}), $(e.currentTarget).parents(".popover").hide()) :void 0;
