...
this.submissionId = options.submissionId, this.submissionOptions = options.submissionOptions, 
...
var challenge_slug, contest_slug, page, that;
return HR.appController.setTitle(this.title_leading + (" Submissions " + submissionId) + this.title_ending), 
this.activeTab = "submissions", challenge_slug = this.model.get("slug"), contest_slug = this.model.get("contest_slug"), 
this.contest || (this.contest = HR.model("contest")), this.contest.set("slug", this.model.get("contest_slug")), 
this.contest.cached(), this.submissionOptions.element && "page" !== this.submissionOptions.element ? void 0 :(page = "page" === this.submissionOptions.element ? this.submissionOptions.element_id :1, 
null === this.submissionOptions._filter && (this.submissionOptions._filter = "all"), 
this.submission_model || (this.submission_model = HR.model("submission", {
id:submissionId,
filter:this.submissionOptions._filter,
contest_slug:this.model.get("contest_slug")
})), this.submission_model.cached(), that = this, HR.requires([ "compound/submission-views" ], function() {
var gameset_collection;
return that.submissionView || (that.submissionView = new HR.SubmissionView({
contest:that.contest,
model:that.submission_model
}), "game" === that.submissionOptions.kind && (gameset_collection = HR.collection("game-set"), 
gameset_collection.setPage(page), gameset_collection.setFilter(that.submissionOptions._filter), 
gameset_collection.setSid(submissionId), gameset_collection.cached(), that.submissionView.setGameSetCollection(gameset_collection))), 
that.submissionView.gameset && that.submissionView.gameset.resetGameSetView(), that.body = that.submissionView, 
that.assign({
".challenge-body":that.body
}), that.setTab("#submissionsTab");
}), window.mixpanel_data.challenge = this.submission_model.slug);
