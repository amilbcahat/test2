...
var editor_theme;
return $(this.el).html("<div id='codeshell-wrapper'></div>"), this.$("#codeshell-wrapper").codeshell({
...
}), editor_theme = $.cookie("hacker_editor_theme"), this.$("#codeshell-wrapper").codeshell("setOption", "theme", this.getThemeName(editor_theme)), 
this.$("#codeshell-wrapper").codeshell("refresh"), this;
...
processData:!1,
success:function(data) {
return btn.setInactive(), data.ok ? that.$("#codeshell-wrapper").codeshell("getOptions").lang_display_mapping[data.data.language] ? (that.$("#codeshell-wrapper").codeshell("setValue", {
language:data.data.language,
code:data.data.source
}), dialog.destroy()) :btn.failed("Language not supported for this challenge") :btn.failed(data.message);
}
});
}
} ], dialog = new HR.util.ShowFormDialog({
