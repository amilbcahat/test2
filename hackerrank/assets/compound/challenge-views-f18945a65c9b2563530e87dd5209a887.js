...
this.submissionId = options.submissionId, this.includePractice = !0, this.leaderboardFilter = options.leaderboardFilter, 
this.contest = new HR.model("contest"), this.contest.set("slug", this.model.get("contest_slug")), 
this.contest.cached(), this.listenTo(this.model, "reset", this.render), this.listenTo(this.contest, "reset", this.render);
...
}, ChallengeView.prototype.setSubmission = function(submissionId, options) {
var contest, page, submission_model, that;
return null == options && (options = {}), contest = HR.model("contest"), options.contest_slug && contest.set("slug", options.contest_slug), 
contest.cached(), options.element && "page" !== options.element ? void 0 :(page = "page" === options.element ? options.element_id :1, 
null === options._filter && (options._filter = "all"), submission_model = HR.model("submission", {
id:submissionId,
filter:options._filter,
contest_slug:options.contest_slug
}).cached(), that = this, HR.requires([ "compound/submission-views" ], function() {
var gameset_collection, submission_view;
return submission_view = new HR.SubmissionView({
contest:contest
}), submission_view.setSubmissionModel(submission_model), "game" === options.kind && (gameset_collection = HR.collection("game-set"), 
gameset_collection.setPage(page), gameset_collection.setFilter(options._filter), 
gameset_collection.setSid(submissionId), gameset_collection.cached(), submission_view.setGameSetCollection(gameset_collection)), 
submission_view.gameset && submission_view.gameset.resetGameSetView(), that.body = submission_view, 
that.assign({
".challenge-body":that.body
}), that.setTab("#submissionsTab"), window.mixpanel_data.challenge = submission_model.slug;
}));
...
}), this.model.get("track") ? $("challenge-nav-track-container").html("<a href='/categories/" + this.model.get("category") + "/" + this.model.get("track").slug + "' class='backbone'>" + this.model.get("track").name + "</a>") :$("challenge-nav-track-container").html(""), 
...
".challenge-body":this.body
})) :"leaderboard" === this.activeTab ? this.setLeaderboard(this.paramPage || 1) :"submissions" === this.activeTab ? this.submissionId ? this.setSubmission(this.submissionId) :this.setSubmissions(this.paramPage || 1) :"forum" === this.activeTab ? this.setForum(this.paramPage || 1) :this.setProblem();
...
ChallengeTemplateView.prototype.className = "challenge-template-view", ChallengeTemplateView.prototype.initialize = function(options) {
return null == options && (options = {}), this.contest = options.contest;
}, ChallengeTemplateView.prototype.render = function() {
var body, events, form_properties, options;
...
}), HR.tour_going && !this.dialog && (HR.tour_going = !1, form_properties = {}, 
options = {
title:"Let's get started!"
}, body = "<div class='plL plR onboarding_modal'> <p class='text-center mlT mlB'> This is the code editor. You can compile your code, view the  leaderboard, submissions, and discussions for the challenge, and submit for scoring when you are ready. Happy Hacking! <!--We have left some helpful hints in case you get stuck.--> </p> </div> <footer class='text-center'><a class='mlB btn btn-green btn-large js-close'>Start solving</a></footer>", 
events = {
"click .js-close":function(e) {
var dialog;
return dialog = e.data.that, dialog.destroy();
}
}, options.body = body, options.events = events, options.width = 640, this.dialog = new HR.util.ShowDialog(options, form_properties), 
...
"change select.translations":"changeLanguage",
"click a.language-preference":"setLanguagepreference"
};
}, ChallengeContentView.prototype.initialize = function(options) {
return null == options && (options = {}), this.model.bind("change", this.render, this), 
this.model.bind("reset", this.render, this), this.contest = options.contest, this.current_hacker = HR.profile();
}, ChallengeContentView.prototype.changeLanguage = function(e) {
var changed_language, filter, render_name_html;
return $("#preference-msg").css("display", "none"), changed_language = $(e.currentTarget).val(), 
filter = _.select(this.model.get("available_translations"), function(translation) {
return translation.language === changed_language;
}), this.current_hacker.get("local_language") || $("#preference-msg").css("display", "inline-block"), 
render_name_html = function(_this) {
return function() {
return 1 === filter.length ? ($(".challenge-text").html(filter[0].body_html || _this.model.get("body_html")), 
$(".hr_tour-challenge-name").html(filter[0].name || _this.model.get("name"))) :($(".challenge-text").html(_this.model.get("body_html")), 
$(".hr_tour-challenge-name").html(_this.model.get("name")));
};
}(this), HR.requires([ "MathJax/MathJax" ], render_name_html), render_name_html();
}, ChallengeContentView.prototype.setLanguagepreference = function(e) {
var changed_language;
return changed_language = $(".translations :selected").text().trim(), e.preventDefault(), 
$("#preference-msg").html("Saving language preference..."), this.current_hacker.set("local_language", changed_language), 
this.current_hacker.save(null, {
success:function() {
return $("#preference-msg").html("Language preference updated."), setTimeout(function() {
return $("#preference-msg").css("display", "none");
}, 3e3);
},
error:function() {
return $("#preference-msg").html("There was some error while saving your preferences.");
}
});
...
contest:_this.contest.toJSON(),
baseURL:_this.model.pageURL()
})), _this.setLanguageSelected(), _this.renderSidebar();
};
...
$(".alert.error").hide();
}, ChallengeContentView.prototype.renderSidebar = function() {
return this.sidebar = new HR.ChallengeSidebarView({
model:this.model
}), this.assign({
".challenge-sidebar-container":this.sidebar
});
...
var form_data, suggestion;
return e.preventDefault(), suggestion = $("#suggestion").val().trim(), suggestion.length < 20 ? ($(".alert.error").show().html("Your suggestion should contain at least 20 characters."), 
void 0) :($(e.currentTarget).html("Processing Request...").attr("disabled", "disabled"), 
form_data = {
...
$("#suggestion-form").slideToggle(), $(".js-suggest-edits").addClass("in");
...
}, ChallengeContentView.prototype.setLanguageSelected = function() {
var current_language;
return current_language = this.model.get("preferred_language") ? this.model.get("preferred_language") :"English", 
$("option[value=" + current_language + "]").attr("selected", "selected");
...
jQuery(function() {
var ChallengeSidebarView, HR;
return ChallengeSidebarView = function(_super) {
function ChallengeSidebarView() {
return ChallengeSidebarView.__super__.constructor.apply(this, arguments);
}
return __extends(ChallengeSidebarView, _super), ChallengeSidebarView.prototype.template = "challenge-sidebar", 
ChallengeSidebarView.prototype.className = "challenge-sidebar-view", ChallengeSidebarView.prototype.events = function() {
return {
"click .share_facebook":"fbshare",
"click .share_twitter":"tweet",
"click .share_gplus":"gplus",
"click .recent-hacker-prev":"recentHackersPrevious",
"click .recent-hacker-next":"recentHackersNext"
};
}, ChallengeSidebarView.prototype.initialize = function(options) {
return null == options && (options = {}), this.model.bind("change", this.render, this), 
this.model.bind("reset", this.render, this), this.page = 1, this.count = 25, this.get_recent_hackers(), 
this.recent_hacker_sync = !1, this.current_hacker = HR.profile();
}, ChallengeSidebarView.prototype.recentHackersPrevious = function() {
return this.recent_hacker_sync ? this.page <= 1 ? void 0 :(this.page = this.page - 1, 
this.get_recent_hackers()) :void 0;
}, ChallengeSidebarView.prototype.recentHackersNext = function() {
return this.recent_hacker_sync ? this.page * this.count >= parseInt(this.model.get("submitted_hackers_count")) ? void 0 :(this.page = this.page + 1, 
this.get_recent_hackers()) :void 0;
}, ChallengeSidebarView.prototype.get_recent_hackers = function() {
var that;
return that = this, this.recent_hacker_sync = !1, this.render(), $.ajax({
url:"" + this.model.restURL() + "/recent_hackers?start=" + (this.page - 1) * this.count,
success:function(data) {
return that.recent_hackers = data, that.recent_hacker_sync = !0, that.render();
}
});
}, ChallengeSidebarView.prototype.render = function() {
return $(this.el).html(HR.appController.template(this.template, this)({
model:this.model.toJSON(),
recent_hackers:this.recent_hackers,
recent_hacker_sync:this.recent_hacker_sync
})), this.$(".recent-hacker-prev").removeClass("disabled"), this.$(".recent-hacker-prev").removeClass("disabled"), 
this.page <= 1 && this.$(".recent-hacker-prev").addClass("disabled"), this.page * this.count >= parseInt(this.model.get("submitted_hackers_count")) && this.$(".recent-hacker-next").addClass("disabled"), 
this;
}, ChallengeSidebarView.prototype.fbshare = function(e) {
...
window.focus();
}, ChallengeSidebarView.prototype.tweet = function(e) {
...
window.focus();
}, ChallengeSidebarView.prototype.gplus = function(e) {
var h, left, permalink, top, url, w;
return permalink = this.getPermaLink(), e.preventDefault(), w = 600, h = 350, left = screen.width / 2 - w / 2, 
top = screen.height / 2 - h / 2, url = "https://plus.google.com/share?url=" + permalink, 
window.open(url, "_blank", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=" + w + ", height=" + h + ", top=" + top + ", left=" + left), 
window.focus();
}, ChallengeSidebarView.prototype.getPermaLink = function() {
...
}(window.HR.GenericView), HR = window.HR || {}, HR.ChallengeSidebarView = ChallengeSidebarView;
});
}.call(this), function() {
var __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
function ctor() {
...
return $(this.el).html(HR.appController.template(this.template, this)()), -1 !== $.inArray(this.model.get("kind"), [ "code", "game", "client-server" ]) && (this.codeeditor || (this.codeeditor = new HR.CodeEditorView({
model:this.model,
parent:this
})), this.assign({
...
model:this.compile_test,
parent:this
})) :"client-server" === this.model.get("kind") && (this.subview || (this.subview = new HR.ClientServerCompileTestView({
model:this.compile_test,
parent:this
}))), this.assign({
...
this.forkable = !1, this.clipboardnotset = !0;
...
var that;
return that = this, this.$("#codeshell-wrapper").codeshell($.extend({
...
enableVersioning:this.profile.isLoggedIn(),
...
total_lines >= line_number && (marker_node = $("<div class='error-marker'><span class='error-marker-icon'>!</span>" + error_message + "</div>"), 
...
var $btn, current_text, dialog, error, that, _timeoutFn;
return that = this, $btn = $(e.currentTarget).find("button.bb-submit"), "disabled" !== $btn.attr("btn-disabled") ? ($btn.attr("btn-disabled", "disabled"), 
$btn.addClass("disabled"), current_text = $btn.html(), $.inArray(this.model.get("id"), HR.CHALLENGES_DISABLED) >= 0 || HR.CONTEST_DISABLED ? (error = new HR.util.ShowSubmitDisabledDialog(), 
...
onDestroy:function() {
return $btn.removeAttr("btn-disabled"), $btn.removeClass("disabled"), $btn.html(current_text);
},
buttons:[ {
...
scrollTop:0
}, 300)) :(btn.setActive(), btn.setFailedMsg(response.message));
},
error:function() {
...
return $btn.html(current_text), setTimeout(_timeoutFn, 100);
},
show_sign_up_link:!0
}), dialog.render(), void 0)) :void 0;
}, CodeEditorView.prototype.setClipboard = function() {
var clipboardcopy, editor, that;
return this.codeshellwrapper = this.$("#codeshell-wrapper"), editor = this.$("#codeshell-wrapper").codeshell("getEditor"), 
