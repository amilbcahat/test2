...
var $btn, dialog;
return $btn = $("#postblog"), "disabled" !== $btn.attr("btn-disabled") ? ($btn.attr("btn-disabled", "disabled"), 
event && event.data && (event.data.status === !0 ? (dialog = new HR.util.ShowConfirmationDialog({
title:"Successfully submitted",
body:"Thank you for submitting your article. We will review and publish it soon."
}), dialog.render(), $(".wp-editor").html("<div class='mjL plT'>We have received your article. We will publish it soon. Thank you.</div>")) :($("#error-message").html(event.data.error).show(), 
$("html, body").animate({
scrollTop:0
}, "slow"))), $btn.removeAttr("btn-disabled")) :void 0;
}, WriteblogView.prototype.postBlog = function(e) {
var data, dialog, sender, that;
return e.preventDefault(), $("#error-message").html("").hide(), that = this, this.profile.isLoggedIn() ? (data = {}, 
data.username = this.profile.get("username"), data.wordpressBaseUrl = this.getWordpressBaseUrl(), 
sender = $("#editor-iframe")[0].contentWindow, sender.postMessage(data, "" + this.getWordpressBaseUrl() + "/editor/editor.php")) :(dialog = new HR.util.ShowLoginDialog({
