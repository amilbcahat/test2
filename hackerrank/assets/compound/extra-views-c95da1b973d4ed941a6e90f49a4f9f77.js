...
return this.profile = HR.profile(), this.id = options.id, console.log("init"), console.log(this.id);
...
return function() {
var data, sender;
return data = {}, data.id = _this.id, data.username = _this.profile.get("username"), 
data.wordpressBaseUrl = HR.util.getWordpressBaseUrl(), data.action = "initMessage", 
sender = $("#editor-iframe")[0].contentWindow, sender.postMessage(data, "" + HR.util.getWordpressBaseUrl() + "/editor/editor.php");
};
}(this));
...
var $btn, dialog;
return $btn = $("#postblog"), console.log(event.data), event && event.data ? "initMessage" === event.data.action ? (event.data.can_edit ? console.log("can edit") :(HR.router.navigate("/blog/new", !0), 
console.log("can't edit")), void 0) :("save" === event.data.action ? ($btn = $("#saveblog"), 
...
var $btn, data, dialog, sender, that;
return e.preventDefault(), $btn = $("#postblog"), "disabled" !== $btn.attr("btn-disabled") ? ($("#error-message").html("").hide(), 
that = this, this.profile.isLoggedIn() ? ($btn.attr("btn-disabled", "disabled"), 
$btn.addClass("disabled"), data = {}, data.id = this.id, data.username = this.profile.get("username"), 
data.wordpressBaseUrl = HR.util.getWordpressBaseUrl(), data.action = "submit", sender = $("#editor-iframe")[0].contentWindow, 
sender.postMessage(data, "" + HR.util.getWordpressBaseUrl() + "/editor/editor.php")) :(dialog = new HR.util.ShowLoginDialog({
body_text:"Please login in order to submit a blog",
...
that = this, this.profile.isLoggedIn() ? ($btn.attr("btn-disabled", "disabled"), 
$btn.addClass("disabled"), data = {}, data.id = this.id, data.username = this.profile.get("username"), 
data.wordpressBaseUrl = HR.util.getWordpressBaseUrl(), data.action = "save", sender = $("#editor-iframe")[0].contentWindow, 
sender.postMessage(data, "" + HR.util.getWordpressBaseUrl() + "/editor/editor.php")) :(dialog = new HR.util.ShowLoginDialog({
body_text:"Please login in order to save a blog",
success_callback:function() {},
show_sign_up_link:!0
}), dialog.render(), void 0)) :void 0;
}, WriteblogView;
