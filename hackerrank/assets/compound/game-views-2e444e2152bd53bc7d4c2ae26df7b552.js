...
moves = this.moves), _.isObject(moves.get("moves")) ? (moves = moves.get("moves"), 
peg_solitaire_id = 739, this.model.get("challenge_id") === peg_solitaire_id && (moves = this.correct_solitaire_moves(moves))) :(moves = this.codechecker_moves, 
...
if (peg_solitaire_id = 739, challenge_id = this.model.get("challenge_id"), parseInt(challenge_id) === peg_solitaire_id) {
if (stdin = this.codechecker_stdin, stdout = this.codechecker_stdout, stderr = this.codechecker_stderr, 
void 0 === moves[0]) return;
if ("CODECHECKER_INVALID_MOVE_ERROR" === moves[0]) return;
for (move_segments = [], move_segments = moves[0].split(","), corrected_moves = [], 
first_couple_moves = move_segments[0].split(":"), corrected_moves[0] = "" + first_couple_moves[0] + ":" + first_couple_moves[1], 
corrected_moves[1] = "" + first_couple_moves[2], stdin.push(stdin[0]), stdout.push(stdout[0]), 
stderr.push(stderr[0]), _ref = move_segments.slice(1), _i = 0, _len = _ref.length; _len > _i; _i++) move = _ref[_i], 
corrected_moves.push(move), stdin.push(stdin[0]), stdout.push(stdout[0]), stderr.push(stderr[0]);
