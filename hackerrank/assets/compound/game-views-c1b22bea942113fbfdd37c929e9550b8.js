...
var cfg, columns, endx, endy, move_objects, moves, num_moves, rows, startx, starty, _i;
...
if (cfg = {
pieceTheme:"assets/games/arimaa/{piece}.png"
}, 0 === i && (this.chessBoard = new ChessBoard("chess-board-wrapper", cfg)), 2 > i || this.current_move > i) this.parent.unSetInfoMessage(), 
rows = moves_array[i].split(":")[1], rows = this.arimaaToChess(rows), this.chessBoard.position(rows); else if (move_objects = moves_array[i].split(":"), 
num_moves = move_objects[2]) for (i = _i = 1; num_moves >= 1 ? num_moves >= _i :_i >= num_moves; i = num_moves >= 1 ? ++_i :--_i) moves = move_objects[2 + i].split(" "), 
startx = rows[moves[0]], starty = columns[moves[1]], endx = rows[moves[2]], endy = columns[moves[3]], 
console.log("" + starty + startx + "-" + endy + endx), this.chessBoard.move("" + starty + startx + "-" + endy + endx), 
-1 !== moves[4] && (startx = rows[moves[4]], starty = columns[moves[5]], endx = rows[moves[6]], 
endy = columns[moves[7]], this.chessBoard.move("" + starty + startx + "-" + endy + endx));
