...
moves || (moves = [])), peg_solitaire_id = 739, this.model.get("challenge_id") === peg_solitaire_id && (moves = this.correct_solitaire_moves(moves)), 
this.move >= moves.length ? (this.setInfoMessage(), this.parent.$("#pause").hide(), 
...
}, GameView.prototype.correct_solitaire_moves = function(moves) {
var challenge_id, corrected_moves, first_couple_moves, move, move_segments, peg_solitaire_id, stderr, stdin, stdout, _i, _len, _ref;
if (peg_solitaire_id = 739, challenge_id = this.model.get("challenge_id"), 0 === moves.length) return [];
if (parseInt(challenge_id) === peg_solitaire_id) {
for (stdin = this.codechecker_stdin, stdout = this.codechecker_stdout, stderr = this.codechecker_stderr, 
move_segments = [], move_segments = moves[0].split(","), corrected_moves = [], first_couple_moves = move_segments[0].split(":"), 
corrected_moves[0] = "" + first_couple_moves[0] + ":" + first_couple_moves[1], corrected_moves[1] = "" + first_couple_moves[2], 
stdin.push(stdin[0]), stdout.push(stdout[0]), stderr.push(stderr[0]), _ref = move_segments.slice(1), 
_i = 0, _len = _ref.length; _len > _i; _i++) move = _ref[_i], corrected_moves.push(move), 
stdin.push(stdin[0]), stdout.push(stdout[0]), stderr.push(stderr[0]);
moves = corrected_moves, this.codechecker_stdin = stdin, this.codechecker_stdout = stdout, 
this.codechecker_stderr = stderr;
}
return moves;
}, GameView;
...
var co_ordinates, columns, dimensions, end_x, end_y, grid, init_grid, move, remove_x, remove_y, rows, start_x, start_y;
if (void 0 !== moves_array[i] && "CODECHECKER_INVALID_MOVE_ERROR" !== moves_array[i]) return move = moves_array[i], 
dimensions = moves_array[0].split(":")[0].split(" "), columns = parseInt(dimensions[0]), 
rows = parseInt(dimensions[1]), init_grid = moves_array[0].split(":")[1], 0 === i ? (grid = "<div class='game-grid' style='width: " + 40 * columns + "px; height: " + 40 * rows + "px;' >", 
_.times(rows, function() {
return function(row) {
var row_el;
return row_el = "<div style='height: 40px; width: " + 40 * columns + "px;' >", _.times(columns, function(column) {
return "x" === init_grid[row * columns + column] ? row_el = row_el.concat('<div data-x="' + column + '" data-y="' + row + '" class="spanhole"> </div>') :"." === init_grid[row * columns + column] ? row_el = row_el.concat('<div data-x="' + column + '" data-y="' + row + '" class="spanhole pegslot"><img src=\'/static/games/peg.png\'></div>') :"-" === init_grid[row * columns + column] ? row_el = row_el.concat('<div data-x="' + column + '" data-y="' + row + '" class="spanhole pegslot"></div>') :void 0;
}), row_el = row_el.concat("</div>"), grid = grid.concat(row_el);
};
}(this)), grid = grid.concat("</div>"), this.grid = grid, this.$(".game-grid-wrapper").html(this.grid)) :(co_ordinates = move.split(" "), 
start_x = parseInt(co_ordinates[0]), start_y = parseInt(co_ordinates[1]), end_x = parseInt(co_ordinates[2]), 
end_y = parseInt(co_ordinates[3]), remove_x = parseInt(co_ordinates[4]), remove_y = parseInt(co_ordinates[5]), 
this.$("[data-x=" + start_x + "][data-y=" + start_y + "]").html(""), this.$("[data-x=" + end_x + "][data-y=" + end_y + "]").html(""), 
this.$("[data-x=" + remove_x + "][data-y=" + remove_y + "]").html(""), this.$("[data-x=" + end_x + "][data-y=" + end_y + "]").html("<img src='/static/games/peg.png'>"));
}, PegSolitaireView;
