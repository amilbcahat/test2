...
short_bio:this.$("input#short_bio").val(),
...
return this.event_collection || (this.event_collection = HR.collection("hacker-event"), 
this.event_collection.setHacker(this.hacker), this.event_collection.cached()), this.events_view || (this.events_view = new HR.HackerEventView({
...
"change #rating-dropdown":"changeChart"
}, RatingHistoryView.prototype.changeChart = function() {
var index, item;
return index = $("#rating-dropdown").val(), index = parseInt(index) - 1, this.tracks[index] ? (item = this.rating[this.tracks[index].text], 
$("#rating-rank").html(item.rank), $("#rating-title").html(item.title), $("#rating-rating").html(item.rating.round()), 
this.renderChart(index)) :void 0;
}, RatingHistoryView.prototype.renderChart = function(index, options) {
var defaultOptions, event, history, prev, that, _i, _len, _ref;
for (null == options && (options = {}), history = this.data.models[index], history.index = index + 1, 
prev = 1500, _ref = history.events, _i = 0, _len = _ref.length; _len > _i; _i++) event = _ref[_i], 
...
var history, index, _i, _len, _ref;
if (this.data && this.model.sync_status) {
for (this.tracks = [], _ref = this.data.models, index = _i = 0, _len = _ref.length; _len > _i; index = ++_i) history = _ref[index], 
this.tracks.push({
id:index + 1,
text:history.category,
count:history.events.length
});
this.rating = this.model.get("current_rating"), $(this.el).html(HR.appController.template(this.template, this)({
model:this.tracks,
rating:this.rating
})), this.changeChart(1);
}
