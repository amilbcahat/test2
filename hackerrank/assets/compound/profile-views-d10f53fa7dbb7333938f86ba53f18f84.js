...
error ? this.$(".alert.error").html(error).parent().fadeIn() :$.ajax({
...
var _callback;
return data.ok ? (that.$(".alert.error").html("<span style='color:green;'>" + data.message + "</span>").parent().fadeIn(), 
that.$(".formgroup input,textarea").val(""), _callback = function() {
return window.location.href = "/login";
}, setTimeout(_callback, 2e3)) :that.$(".alert.error").html(data.message).parent().fadeIn();
}
});
