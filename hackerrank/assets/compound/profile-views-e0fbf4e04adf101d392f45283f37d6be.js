...
merge.save({
username:this.$("#mergeaccounts").val()
}, {
success:function(_this) {
return function() {
return _this.$(".merge-response").addClass("success").removeClass("error").html("Great! We have sent a confirmation email to " + username + " and " + _this.$("#mergeaccounts").val() + ". Please click the link in both of those emails to confirm the merge.");
};
}(this),
error:function(_this) {
...
silent:!0
}), this.model.save(), this.render(), $("#save-success").show();
}, SettingsAccountView;
...
error ? this.$(".error-block").html(error).parent().fadeIn() :$.ajax({
...
}), this.model.save(), $("#save-success").css("display", "inline-block");
...
return $(this.el).find("#country").val(country), $(this.el).find("#s2id_country .select2-choice span.select2-chosen").text(country);
...
return e.preventDefault(), this.subview.saveChanges(e);
