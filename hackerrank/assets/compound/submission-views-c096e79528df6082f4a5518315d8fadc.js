...
SubmissionView.prototype.className = "submission-individual-view container", SubmissionView.prototype.events = {
"click .copy_to_clipboard":"CopyToClipBoard"
}, SubmissionView.prototype.initialize = function(options) {
return this.contest = options.contest, this.has_template = !1, this.model && this.listenTo(this.model, "reset", this.render), 
this.render();
...
return this.has_template ? this.renderCurrentView() :(this.has_template = !0, this._template = this.template, 
this.renderCurrentView()), this;
}, SubmissionView.prototype.renderCurrentView = function() {
var $code_area, editorOptions, showBusyThrobber, that;
return this._template && this.model.sync_status ? ($(this.el).html(HR.appController.template(this._template, this)({
model:this.model.toJSON(),
_model:this.model,
...
}(this))), this.renderSubViews()) :(this._ThrobberTimer && window.clearTimeout(this._ThrobberTimer), 
...
return this.model && this.model.sync_status !== !0 ? this :(this.$(".apply-blob") && (this.apply_blob_view ? (this.apply_blob_view.setElement(this.$(".apply-blob")), 
...
parent:this
})), this.stats.setModel(this.model), "game" === this.model.get("kind") && this.collection && this.collection.sync_status && (this.gameset || (this.gameset = new HR.SubmissionGameCollectionView({
parent:this,
model:this.model,
collection:this.collection
...
}, SubmissionStatsView.prototype.initialize = function(options) {
return $(".change-filter").live("click", this.updateSelect), this.parent = options.parent, 
this.render();
...
return Backbone.history.navigate("" + HR.appController.get_current_contest_namespace() + "/challenges/" + that.parent.model.get("challenge_slug") + "/submissions/game/" + that.model.get("id") + "/" + $("#submissions-filter").select2("data").slug, !0);
});
}
...
HR.util.pagination(this.$(".pagination-wrapper"), this._collection.total, "" + HR.appController.get_current_contest_namespace() + "/challenges/" + this.parent.model.get("challenge_slug") + "/submissions/game/" + this.model.get("id") + "/" + this.collection.getFilter() + "/page/", this._collection.page, null, 5, 10, "change-gameset"), 
...
jQuery(function() {
var HR, SubmissionGameCollectionView, _ref;
return SubmissionGameCollectionView = function(_super) {
function SubmissionGameCollectionView() {
return SubmissionGameCollectionView.__super__.constructor.apply(this, arguments);
}
return __extends(SubmissionGameCollectionView, _super), SubmissionGameCollectionView.prototype.template = "submission-game-collection", 
SubmissionGameCollectionView.prototype.className = "submission-game-collection-view", 
SubmissionGameCollectionView.prototype.initialize = function(options) {
...
}, SubmissionGameCollectionView.prototype.events = {
"click .show-gameset-button":"showGameContainer"
}, SubmissionGameCollectionView.prototype.render = function() {
return window.SubDebugListView = this, $(this.el).html(HR.appController.template(this.template, this)({
model:this.model.toJSON(),
_model:this.model,
collection:this.collection.toJSON(),
_collection:this.collection
})), this.collection.sync_status ? (this.$(".throbber-row").hide(), this.$(".game-view-rows").show(), 
this.$(".zero-content").hide(), 0 === this.collection.length && (this.model.get("filter") ? this.$(".zero-content-" + this.model.get("filter")).show() :this.$(".zero-content-all").show())) :(this.$(".game-view-rows").hide(), 
this.$(".throbber-row").show()), this.setPagination(), this;
}, SubmissionGameCollectionView.prototype.setPagination = function() {
var that, _filter;
return _filter = this.collection.getFilter(), HR.util.pagination(this.$(".pagination-wrapper"), this.collection.total, "" + HR.appController.get_current_contest_namespace() + "/challenges/" + this.parent.model.get("challenge_slug") + "/submissions/game/" + this.model.get("id") + "/" + this.collection.getFilter() + "/page/", this.collection.page, null, 5, 10, "change-gameset"), 
that = this, this.$(".pagination a.change-gameset").click(function(e) {
var href, page;
return e.preventDefault(), page = parseInt($(e.currentTarget).attr("data-page")), 
href = $(e.currentTarget).attr("href"), HR.router.navigate(href, !1), that.collection.page = page, 
that.collection.fetch({
reset:!0
}), that.$(".game-view-rows").hide(), that.$(".throbber-row").show();
}), this;
}, SubmissionGameCollectionView.prototype.showGameContainer = function(e) {
var id, that;
return e.preventDefault(), this.gameContainerView && this.gameContainerView.remove(), 
this.$(".show-gameset-container").html(""), this.$(".show-gameset-container").hide(), 
id = $(e.currentTarget).attr("data-id"), this.gameModel = this.collection.findWhere({
id:id
}), that = this, HR.requires([ "compound/game-views" ], function() {
var gameCollection;
return that.$(".show-gameset-container-" + id).show(), gameCollection = new HR.GameCollection(that.gameModel.get("games")), 
that.gameContainerView = new HR.GameContainerView({
collection:gameCollection,
gameset:that.gameModel
}), that.gameContainerView.setElement(that.$(".show-gameset-container-" + id)).render(), 
that.add_subview(that.gameContainerView);
});
}, SubmissionGameCollectionView;
}(window.HR.GenericView), HR = null != (_ref = window.HR) ? _ref :{}, HR.SubmissionGameCollectionView = SubmissionGameCollectionView;
});
}.call(this), function() {
var __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
function ctor() {
...
return this.contest = options.contest, this.parent = options.parent;
...
return that.contest && that.contest.get("id") && 1 !== that.contest.get("id") ? HR.router.navigate("/contests/" + that.contest.get("slug") + "/challenges/" + this.parent.model.get("challenge_slug") + "/submissions/" + that.model.get("kind") + "/" + that.model.get("id") + "/page/" + data.page, !0) :HR.router.navigate("/challenges/" + this.parent.model.get("challenge_slug") + "/submissions/" + that.model.get("kind") + "/" + that.model.get("id") + "/page/" + data.page, !0), 
