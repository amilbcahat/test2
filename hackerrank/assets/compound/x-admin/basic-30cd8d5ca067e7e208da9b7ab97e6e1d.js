...
var request_params;
return e.preventDefault(), request_params = {
url:"/xrest/admin/tests/invite_candidates",
type:"POST",
dataType:"json",
success:function(_this) {
return function(xhr) {
return xhr.status === !0 ? _this.$(".message-box").html("" + xhr.invited + " candidates have been invited").removeClass("hidden").removeClass("error").addClass("success") :_this.$(".message-box").html("" + xhr.message).removeClass("hidden").removeClass("success").addClass("error");
};
}(this),
error:function() {
return function(xhr) {
return console.log("error"), console.log(xhr);
};
}(this)
