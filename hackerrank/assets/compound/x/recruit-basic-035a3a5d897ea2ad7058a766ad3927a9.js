...
this.showPermissions()) :"add to team" === this.type ? (name = "select2", this.$("input[name=user-email]").attr("disabled", "disabled"), 
this.$("input[name=user-teams-select]").attr("disabled", "disabled"), this.$("input[name=user-teams-select]").val(this.collection.get(this.team_id).get("name")), 
this.$("select[name=user-role]").attr("disabled", "disabled"), this.$("input[name=user-admin]").attr("disabled", "disabled"), 
multiple = !1, disabled = !0, this.showPermissions()) :"edit" === this.type && (name = "pre-filled", 
...
this.$("input[name=user-admin]").removeAttr("checked").addClass("hidden"))), questions_permission = this.user_data.questions_permission_json, 
tests_permission = this.user_data.tests_permission_json, candidates_permission = this.user_data.candidates_permission_json, 
interviews_permission = this.user_data.interviews_permission_json, this.showPermissions(questions_permission, tests_permission, candidates_permission, interviews_permission)), 
setTimeout(function(_this) {
return function() {
return "add user" === _this.type ? _this.$("input[name=user-teams-select]").select2({
data:teams,
multiple:!0
}) :void 0;
};
...
return null == questions_permission && (questions_permission = null), null == tests_permission && (tests_permission = null), 
null == candidates_permission && (candidates_permission = null), null == interviews_permission && (interviews_permission = null), 
questions_permission || (questions_permission = {
owned:this.collection.new_user_permissions.owned.questions_permission,
shared:this.collection.new_user_permissions.shared.questions_permission
}), tests_permission || (tests_permission = {
owned:this.collection.new_user_permissions.owned.tests_permission,
shared:this.collection.new_user_permissions.shared.tests_permission
}), candidates_permission || (candidates_permission = {
owned:this.collection.new_user_permissions.owned.candidates_permission,
shared:this.collection.new_user_permissions.shared.candidates_permission
}), interviews_permission || (interviews_permission = {
owned:this.collection.new_user_permissions.owned.interviews_permission,
shared:this.collection.new_user_permissions.shared.interviews_permission
...
}, permissions.questions_permission.shared = this.$("input[name=questions-permission]").is(":checked") ? this.defaultPermissions.questions_permission.shared >= 2 ? this.defaultPermissions.questions_permission.shared :2 :this.defaultPermissions.questions_permission.shared <= 1 ? this.defaultPermissions.questions_permission.shared :1, 
permissions.tests_permission.owned = this.$("input[name=tests-permission][value=create-tests]").is(":checked") ? 3 :0, 
permissions.tests_permission.shared = this.$("input[name=tests-permission][value=share-tests]").is(":checked") ? this.defaultPermissions.tests_permission.shared >= 2 ? this.defaultPermissions.tests_permission.shared :2 :this.defaultPermissions.tests_permission.shared <= 1 ? this.defaultPermissions.tests_permission.shared :1, 
permissions.candidates_permission.shared = this.$("input[name=candidates-permission][value=share-candidates]").is(":checked") ? this.defaultPermissions.candidates_permission.shared >= 2 ? this.defaultPermissions.candidates_permission.shared :2 :0, 
permissions.interviews_permission.owned = this.$("input[name=interviews-permission][value=schedule-interviews]").is(":checked") ? 3 :0, 
permissions.interviews_permission.shared = this.$("input[name=interviews-permission][value=share-interviews]").is(":checked") ? this.defaultPermissions.interviews_permission.shared >= 2 ? this.defaultPermissions.interviews_permission.shared :2 :0, 
permissions;
...
this.$(".js-search-container").css("margin-top", "0px")) :(this.$(".js-search-container").addClass("hidden"), 
...
var action, data, team, team_id, user_ids, user_names, users_checboxes;
...
user_ids = [], user_names = [], _.each(users_checboxes, function() {
return function(checkbox) {
var user_id;
return user_id = $(checkbox).data("user-id"), user_ids.push(user_id), _.each(team.get("users"), function(user) {
return user.get("user_id") === user_id ? user_names.push(user.get("firstname") + " " + user.get("lastname")) :void 0;
});
};
}(this)), data = {
user_ids:user_ids,
...
var action, data, team_id, user_ids, user_names, users_checboxes;
return e.stopPropagation(), e.preventDefault(), action = this.$(e.currentTarget).data("action"), 
team_id = this.collection.models[0].get("id"), users_checboxes = this.$("input[name=search-user]:checked"), 
user_ids = [], user_names = [], _.each(users_checboxes, function() {
return function(checkbox) {
var user_id;
return user_id = $(checkbox).data("user-id"), user_ids.push(user_id);
};
}(this)), data = {
user_ids:user_ids,
team_id:team_id,
...
var action, add_to_team_modal, team_id, user_ids, user_names;
return user_ids = data.user_ids, team_id = data.team_id, action = data.action, console.log(action), 
"add-to-team" === action ? (console.log("here"), console.log(this.$(".dialog-wrapper").length), 
add_to_team_modal = new HR.AddUsersToTeamView({
parent:this,
collection:this.collection,
user_ids:user_ids,
team_id:team_id
}), this.subviews.push(add_to_team_modal), this.$(".dialog-wrapper").html(add_to_team_modal.render().el)) :"remove-from-team" === action ? (user_names = data.user_names, 
HR.util.confirm({
title:"Confirm removing user(s)",
message:"Are you sure you want to remove " + HR.util.arrayToString(user_names) + " from " + this.collection.get(team_id).get("name") + " team?",
okButtonText:"Yes.",
cancelButtonText:"No, go back.",
okCallback:function(_this) {
return function() {
return $.ajax({
url:"/xrest/teams/" + team_id + "/users/batch_ops",
dataType:"JSON",
type:"POST",
data:{
user_ids:user_ids,
...
$.ajax({
url:"/xrest/teams/" + this.team_id + "/users/batch_ops",
dataType:"JSON",
type:"POST",
data:{
...
return that = this, _collection = this.collection.toJSON(), can_create_tests = HR.currentUser && HR.currentUser.get("permissions") && 0 === parseInt(HR.currentUser.get("permissions").tests_permission, 10) ? !1 :!0, 
