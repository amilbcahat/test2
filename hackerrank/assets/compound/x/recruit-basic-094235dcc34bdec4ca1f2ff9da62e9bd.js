...
that.data = json, fnCallback(json), $(".tip").tooltip();
}
});
}
};
...
return [ {
name:"CANDIDATE",
template:'<span class="sleekstrong"><%= row.candidate() ? row.candidate().name : "-" %></span>',
...
var result, that;
return that = this, ev.preventDefault(), result = _.filter(this.data.models, function(row) {
return row.get("id").toString() === $(ev.target).attr("data-interview");
}), result.length ? ($("#cancel-dialog-modal").modal(), $("#cancel-dialog-modal").delegate(".save-button", "click", function() {
return $("#cancel-dialog-modal").modal("hide"), $("#cancel-dialog-modal").undelegate(".save-button", "click"), 
result[0].destroy({
success:function() {
return that.redraw();
}
...
"click a.cancel":"cancelInterview"
}), ob;
...
return e.preventDefault(), submitUrl = "/xrest/users/upload_logo", $form = this.$("#logo-upload-form"), 
...
return data = $.parseJSON(data.responseText), HR.currentUser.fetch({
silent:!0,
success:function() {
return that.$("#upload-logo-modal").modal("hide"), that.parent.render();
}
});
}).error(function(data) {
return that.$(".error").html(data.message);
});
}, LogoUploadView;
...
}(this)), this.$(this.country_select_box).select2({
data:data
});
...
data.user.country = this.$(this.country_select_box).val(), data.user.timezone = this.$(this.timezone_select_box).val(), 
...
return console.log(model), console.log("success");
},
...
var data, errorMsg, promise, that;
return this.$(".error").addClass("hidden"), e.preventDefault(), that = this, data = {}, 
data.user = {}, data.user.old_password = this.$("#usr-old-pwd").val(), data.user.new_password = this.$("#usr-new-pwd").val(), 
data.user.confirm_password = this.$("#usr-cnfm-pwd").val(), data.user.new_password === data.user.confirm_password ? (promise = this.model.updatePassword(data.user), 
promise.success(function(response) {
return console.log(response);
}), promise.error(function(response) {
return console.log(response);
})) :(errorMsg = "Passwords do not match", this.$("#usr-cnfm-pwd").siblings(".error").html(errorMsg).removeClass("hidden"));
...
return e.preventDefault(), confirm("Are You sure you want to delete this test ?") ? this.model.destroy({
success:function() {
return Backbone.history.navigate("tests", !0);
},
error:function() {
return function(model, xhr) {
var response;
...
jQuery(function() {
var HR, MCQChoiceView, QuestionCodeEditStep1View, QuestionCodeEditStep2View, QuestionCodeEditStep3View, QuestionCodeEditStep4View, QuestionCodeEditView, QuestionCompleteEditView, QuestionMCQEditView, QuestionSubjectiveEditView, TestQuestionEditView, TestcaseAlertModalView, TestcaseEditModalView, TestcaseView, TestcasesUploadModalView, _ref;
...
return this.index = options.index, this.listenTo(this.model, "change", this.render);
...
var score, that;
return score = this.$(".score-input").val(), score === !this.question.score ? (this.cancelEditScore(), 
...
return e.preventDefault(), this.$(".score-changer").addClass("hidden"), this.$(".score-input").css("display", "inline"), 
this.$(".score-input").removeClass("hidden"), this.$(".score-input").val(this.question.score), 
this.$(".score-input").focus();
...
return null == options && (options = {}), this.parent = options.parent, this.listenTo(this.model, "change", this.render), 
this.listenTo(this.model, "reset", this.render);
...
var attrs, message_name, saved_messages, _ref;
return message && (message_name = prompt("Enter name to save this message as", "")) ? (saved_messages = null != (_ref = this.model.get("saved_messages")) ? _ref :{}, 
saved_messages[message_name] = message, attrs = {
saved_messages:saved_messages
}, this.model.save(attrs)) :void 0;
...
var attrs, check, saved_messages, _ref;
return message && (check = confirm("This will overwrite the old saved response. Are You sure?", "")) ? (saved_messages = null != (_ref = this.model.get("saved_messages")) ? _ref :{}, 
saved_messages[name] = message, attrs = {
saved_messages:saved_messages
}, this.model.save(attrs)) :void 0;
}, TestSavedResponse.prototype.deleteMessage = function(name) {
var attrs, check, saved_messages, _ref;
return check = confirm("Are you sure you want to delete this saved response ?", ""), 
check && (saved_messages = null != (_ref = this.model.get("saved_messages")) ? _ref :{}) ? (delete saved_messages[name], 
attrs = {
saved_messages:saved_messages
}, this.model.save(attrs), this.render()) :void 0;
}, TestSavedResponse;
