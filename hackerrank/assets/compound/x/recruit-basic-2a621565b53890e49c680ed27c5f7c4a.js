...
return starttime = new Date(moment.tz($(".date.start").val() + " " + $(".time.start").val(), $("#timezones").val()).format()), 
...
this.$(".filter-status").addClass("mjL");
...
}(this)), this.model.set("interviewers", interviewers), starttime = new Date(moment.tz($(".date.start").val() + " " + $(".time.start").val(), $("#timezones").val()).format()), 
duration = this.$("select[name=interview-duration]").val(), end = moment(starttime) + 6e4 * parseInt(duration), 
...
this.$(".lang-container").removeClass("hidden"), this.$(".run-time").html(HR.util.formatTimeInUserTz(current_recording.time, "M-D-YYYY HH:mm")), 
...
void 0) :("email" === column ? ret.push(row.email) :"link" === column ? ret.push("<div class='btn-group'><a style='min-width:80px; padding:8px;' class='btn btn-mini mdL js-invaction' data-id='" + row.id + "' data-action='cancel'>Cancel invite</a><a style='min-width:80px; padding:8px;' class='btn btn-mini mdL js-invaction' data-action='reinvite' data-id='" + row.email + "'>Reinvite</a></div>") :"invited_by" === column ? ret.push(row.invited_by) :"endtime" === column ? ret.push(HR.util.formatTimeInUserTz(row[column], "D MMM, YYYY - HH:mm")) :ret.push(""), 
