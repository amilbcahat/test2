...
var content, track_data;
return content = HR.appController.template(this.template, this)({
test:this.testmodel.attributes,
filter:this.filter,
library:this.library
}), $(this.el).html(content), track_data = {
library:this.library,
step:"Start",
type:this.filter,
test_id:this.testmodel.get("id")
}, HR.util.track("Added question from library", track_data), setTimeout(this.loadLib, 200), 
this;
...
var question_view, track_data;
switch (_.each(this._subviews, function() {
return function(view) {
return view.destroy();
};
}(this)), this.model.get("id") ? (track_data = {
type:this.model.get("type"),
test_id:this.testmodel.get("id"),
question_id:this.model.get("id")
}, HR.util.track("Edited question", track_data)) :(track_data = {
step:"Start",
type:this.model.get("type"),
test_id:this.testmodel.get("id")
}, "code" === this.model.get("type") || "approx" === this.model.get("type") ? HR.util.track("Added new coding question", track_data) :HR.util.track("Added new other question", track_data)), 
this.model.get("type")) {
case "code":
case "approx":
