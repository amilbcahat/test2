...
void 0) :("email" === column ? ret.push(row.email) :"link" === column ? ret.push("<div class='btn-group'><a style='min-width:80px; padding:8px;' class='btn btn-mini mdL js-invaction' data-id='" + row.id + "' data-action='cancel'>Cancel invite</a><a style='min-width:80px; padding:8px;' class='btn btn-mini mdL js-invaction' data-action='reinvite' data-id='" + row.id + "' data-email='" + row.email + "'>Reinvite</a></div>") :"invited_by" === column ? ret.push(row.invited_by) :"endtime" === column ? ret.push(HR.util.formatTimeInUserTz(row[column], "D MMM, YYYY - HH:mm")) :ret.push(""), 
...
void 0) :"invited_by" === column ? (ret.push(row.invited_by), void 0) :"endtime" === column ? (ret.push(HR.util.formatTimeInUserTz(row[column], "D MMM, YYYY - HH:mm")), 
...
TestTimeSettingsView.prototype.timezone_select_box = "#usr-tz-select2", TestTimeSettingsView.prototype.events = function() {
return {
...
timeFormat:"H:i",
scrollDefaultNow:!0
}), this.initTimeZoneSelect2()), this;
}, TestTimeSettingsView.prototype.initTimeZoneSelect2 = function() {
var that;
return that = this, setTimeout(function() {
return $.ajax({
url:"/xrest/users/timezones",
...
var attrs, end, enddate, endtime, start, startdate, starttime, timezone;
return e.preventDefault(), timezone = this.$(this.timezone_select_box).val(), startdate = $("#date-from").val(), 
startdate && ("" === $.trim($(".time.start").val()) ? (starttime = "00:00", $(".time.start").val(starttime)) :starttime = $.trim($(".time.start").val()), 
starttime += ":00", start = startdate + " " + starttime), enddate = $("#date-to").val(), 
enddate && ("" === $.trim($(".time.end").val()) ? (endtime = "00:00", $(".time.end").val(endtime)) :endtime = $.trim($(".time.end").val()), 
endtime += ":00", end = enddate + " " + endtime), attrs = {
starttime:start,
endtime:end,
timezone:timezone
}, this.model.save(attrs, {
