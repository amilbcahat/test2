...
trigger:"none"
}), client = new ZeroClipboard($("#copy-access-code")), client.on("complete", function() {
var elem;
...
ret["interview_attendants.email"] = this.$("#interviewer").val()), "0" !== this.$("#owner").val() && (ret["user.email"] = this.$("#owner").val()), 
...
}(this)), this.$("#owner").select2({
width:"250px"
}), this.$("#owner").on("change", function(_this) {
return function() {
return _this.redraw();
};
}(this)), this.$("#applyFilters").on("click", function(_this) {
return function() {
return _this.redraw();
};
...
return row.email ? _this.$("#interviewer").append('<option value="' + row.email + '">' + row.name + " - <br/>" + row.email + "</option>") :void 0;
});
};
}(this)
}), $.ajax({
url:"" + HR.INTERVIEWS_DOMAIN + "/api/interviews/owners?api_key=" + HR.currentUser.get("apiKey") + "&quickpad=" + this.quickpad,
data:{
"interview.from":this.baseParams().from
},
dataType:"json",
...
return function(data) {
return _.each(data.results, function(row) {
return row.email ? _this.$("#owner").append('<option value="' + row.email + '">' + row.firstname + " - <br/>" + row.email + "</option>") :void 0;
});
};
}(this)
});
...
url:"" + HR.INTERVIEWS_DOMAIN + "/api/interviews/done?api_key=" + HR.currentUser.get("apiKey"),
...
var customDatatableOpt;
return console.log(options), customDatatableOpt = {
oLanguage:{
...
console.log(options), InterviewsQuickPadDataTableView.__super__.initialize.call(this, options);
...
template:'<a href="interviews/<%= row.get("id") %>" class="txt-alt-grey mdL js-backbone"><i class="icon2-edit tip" data-original-title="Edit"></i></a><a href="interviews/<%= row.get("id") %>/report" class="txt-alt-grey mdL js-backbone"><i class="icon-eye tip" data-original-title="View Report"></i></a><a href="#" class="txt-alt-grey mdL cancel js-delete-interview" data-interview="<%= row.get("id") %>"><i class="icon2-delete tip" data-original-title="Delete"></i></a>',
sortable:!1,
className:"w10",
sortable:!1
} ];
...
return InterviewsQuickPadDataTableView.__super__.postRender.call(this), this.$(".filter-interviewer").hide(), 
this.$(".filter-quickpad").addClass("mjL");
...
console.log(options), InterviewsUpcomingDataTableView.__super__.initialize.call(this, options);
...
}), $("#global-zeroclipboard-html-bridge").css("position", "fixed"), this.$("#copy-share-link-report").popover({
...
}, HR.util.track("Added question from library", track_data), HR.util.track("Added question", track_data), 
el.removeClass("js-add-question btn-primary"), el.addClass("js-remove-question btn-alert"), 
...
track_data.question_id = model.get("id"), HR.util.track("Added question", track_data)), 
Backbone.history.navigate("tests/" + _this.testmodel.get("id") + "/questions/" + model.get("id") + "/edit", !0);
};
}(this),
error:function(model, response) {
...
return _.isEmpty(track_data) || (HR.util.track("Added new other question", track_data), 
track_data.question_id = model.get("id"), HR.util.track("Added question", track_data)), 
Backbone.history.navigate("tests/" + that.testmodel.get("id") + "/questions/" + that.model.get("id") + "/edit", !0);
},
error:function() {
...
var question, save_data, track_data;
...
track_data.question_id = model.get("id"), HR.util.track("Added question", track_data)), 
Backbone.history.navigate("tests/" + _this.testmodel.get("id") + "/questions/" + model.get("id") + "/edit/step2", !0);
};
}(this)
}));
...
this.$(".sharereport-modal").modal(), this.$("#report-link-inp").select(), this.$("#copy-share-link-report").popover({
content:"Link has been copied",
trigger:"none"
}), client = new ZeroClipboard($("#copy-share-link-report"), {
...
url += "?api_key=" + HR.currentUser.get("apiKey") + "&_=" + 1e7 * Math.random();
...
return function() {
return _this.$("input[type=file]").bootstrapFileInput();
};
}(this), 10), this.set_answer(), window.view = this, this;
...
ans && (dat.to_save_code = ans, dat.qid = HR.candidate.questionView.model.get("unique_id"))), 
