...
input = m.get("stdin")[0], output = m.get("stdout")[0], st_class = "green", HR.candidate.ctview.addTestCase(1, input, output, null, "", st_class);
...
this.$("#search-loading").hide(), "" !== this.advacned_search.filterString() ? this.$("#tests-title").text("Tests matching search") :this.$("#tests-title").text("All Tests"), 
this.advanced_search_open ? this.showAdvanceSearch() :this.hideAdvanceSearch();
...
return function(xhr) {
var nextpath;
return xhr.error ? _this.showError(xhr.error) :(nextpath = _this.model.get("nextqid") ? "" + _this.tid + "/questions/" + _this.model.get("nextqid") :"" + _this.tid + "/questions", 
...
message:err,
callback:function() {
return HR.router.navigate("" + uid + "/redirect", {
trigger:!0,
replace:!0
});
}
...
this.$(".ans-submit").addClass("disabled")), HR.util.scrollToTop(), this;
...
var email, pass, that;
this.disableButton("test-submit"), that = this, e.preventDefault(), this.resetError(), 
email = this.$("input[name=username]").val(), pass = this.$("input[name=password]").val(), 
this.$("#acknowledge").is(":checked") || (this.enableButton("test-submit"), this.$("#login-form").before('<div class="text-center alert error" id="acknowledge-alert"> You cannot take this test without agreeing to the specified conditions. </div>'));
