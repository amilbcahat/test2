...
success:function() {
return HR.requires([ "compound/submission-views" ], function() {
var challenge_model, gameset_collection, submission_view;
return submission_view = new HR.SubmissionView({
contest:contest
}), submission_view.setSubmissionModel(submission_model), "game" === kind && (gameset_collection = HR.collection("game-set"), 
gameset_collection.setPage(page), gameset_collection.setFilter(_filter), gameset_collection.setSid(submission_id), 
gameset_collection.cached(), submission_view.setGameSetCollection(gameset_collection)), 
submission_view.gameset && submission_view.gameset.resetGameSetView(), console.log(submission_model.get("challenge_slug")), 
challenge_model = HR.model("challenge", {
slug:submission_model.get("challenge_slug")
}).cached(), HR.requires([ "compound/challenge-views" ], function() {
var challenge_view;
return challenge_view = new HR.ChallengeView({
model:challenge_model,
customView:submission_view,
activeTab:"submissions"
}), HR.appView.setContentView(challenge_view);
});
});
