...
var eventDate;
return eventDate = new Date(event.date), startDate && startDate > eventDate ? (console.log(eventDate, startDate), 
!1) :endDate && eventDate > endDate ? !1 :!0;
}), defaultOptions = {
chart:{
zoomType:"x",
backgroundColor:null,
...
formatter:function() {
var colors, date, html, level_in_words;
return event = this.point.event, date = new Date(event.date), level_in_words = [ "O(1)", "O(1)", "O(log N)", "O(N)", "O(N^2)", "O(2^N)" ], 
colors = [ "#2bc56d", "#2bc56d", "#32a1de", "#f65039", "#805ec9", "#c2c7d0" ], html = 'Level: <span style="color:' + colors[event.level] + '">' + level_in_words[event.level] + "</span><br/>Score: " + event.rating;
}
}
