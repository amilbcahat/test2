...
return __extends(CommonController, _super), CommonController.prototype.basic_codemirror_addons = [ "codemirror/lib/codemirror", "codemirror/keymap/vim", "codemirror/keymap/emacs", "codemirror/addon/comment/comment", "codemirror/addon/edit/closebrackets", "codemirror/addon/edit/matchbrackets", "codemirror/addon/fold/foldcode", "codemirror/addon/fold/foldgutter", "codemirror/addon/fold/brace-fold", "codemirror/addon/fold/xml-fold", "codemirror/addon/fold/markdown-fold", "codemirror/addon/fold/comment-fold", "codemirror/addon/fold/indent-fold", "codemirror/addon/runmode/runmode", "codemirror/addon/hint/show-hint" ], 
CommonController.prototype.loadCodeMirror = function(callback, onerror) {
return require(this.basic_codemirror_addons, function() {
return function(CodeMirror) {
return window.CodeshellFactory($, window, _, CodeMirror), callback();
};
}(this), function() {
return function() {
...
return require(this.basic_codemirror_addons, function(CodeMirror) {
...
return function() {
return _this.CodeMirror = CodeMirror, callback();
};
}(this)), args.push(function(_this) {
return function() {
return _this.CodeMirror = CodeMirror, callback();
};
...
"" + base_path + "/" + path;
}, AppController.prototype.addTemplate = function(logical_path, template) {
return HR.templates[logical_path] = template;
}, AppController.prototype.requires = function() {
var callback, errorCallback, number_paths, postCallbacks, preCallbacks, staticFiles, _callback;
...
preCallbacks = [], postCallbacks = [], staticFiles = _.map(_.toArray(arguments).slice(0, number_paths), function() {
return function(path) {
var inject_mathjax_config, _callback;
return _callback = callback, "MathJax/MathJax" === path ? (0 === $("#mathjax-config").length && (inject_mathjax_config = "<script id=\"mathjax-config\" type=\"text/x-mathjax-config\">\n  MathJax.Hub.Config({\n    tex2jax: {\n        inlineMath: [['$','$']],\n        processEscapes: true\n      }\n  });\n</script>", 
...
return callback();
}), callback(), _.each(postCallbacks, function(callback) {
return callback();
});
}, require(staticFiles, _callback, errorCallback);
...
null == options && (options = {}), prev = 600, _ref = history.events, _i = 0, _len = _ref.length; _len > _i; _i++) event = _ref[_i], 
...
var current_rating;
return current_rating = this.model.get("current_rating"), _.isObject(current_rating) && _.isObject(current_rating[track_val]) ? (current_rating[track_val].rating ? this.$("#hacker-rating").html(parseInt(current_rating[track_val].rating)) :this.$("#hacker-rating").html("N/A"), 
current_rating[track_val].rank ? this.$("#hacker-rank").html(current_rating[track_val].rank) :this.$("#hacker-rank").html("N/A"), 
current_rating[track_val].title ? (this.$("#hacker-title").html(current_rating[track_val].title), 
this.$("#hacker-title").removeClass("color-green color-blue color-orange color-purple color-navy"), 
this.$("#hacker-title").addClass(this.color(current_rating[track_val].title))) :this.$("#hacker-title").html("N/A")) :(this.$("#hacker-rating").html("N/A"), 
this.$("#hacker-rank").html("N/A"), this.$("#hacker-title").html("N/A"));
}, RatingHistoryView.prototype.color = function(title) {
return "O(1)" === title ? "color-green" :"O(logN)" === title ? "color-blue" :"O(N)" === title ? "color-orange" :"O(N<sup>2</sup>)" === title ? "color-purple" :"color-navy";
