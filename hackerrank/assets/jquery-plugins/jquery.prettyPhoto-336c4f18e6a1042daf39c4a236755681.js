...
_getDimensions(imageWidth, imageHeight), (pp_containerWidth > windowWidth || pp_containerHeight > windowHeight) && _fitToViewport(pp_containerWidth, pp_containerHeight);
}
return {
...
$.browser.msie && 7 == $.browser.version && (detailsHeight += 8), $pp_details.remove(), 
$pp_title = $pp_pic_holder.find(".ppt"), $pp_title.width(width), titleHeight = parseFloat($pp_title.css("marginTop")) + parseFloat($pp_title.css("marginBottom")), 
...
projectedTop = windowHeight / 2 + scroll_pos.scrollTop - contentHeight / 2, 0 > projectedTop && (projectedTop = 0), 
...
isSet && settings.overlay_gallery && "image" == _getFileType(pp_images[set_position]) && settings.ie6_fallback && (!$.browser.msie || 6 != parseInt($.browser.version)) ? (itemWidth = 57, 
...
}), settings.allow_expand && $("a.pp_expand").bind("click", function() {
...
settings.ie6_fallback && $.browser.msie && 6 == parseInt($.browser.version) && (settings.theme = "light_square"), 
theRel = $(this).attr(settings.hook), galleryRegExp = /\[(?:.*)\]/, isSet = galleryRegExp.exec(theRel) ? !0 :!1, 
pp_images = isSet ? jQuery.map(matchedObjects, function(n) {
return -1 != $(n).attr(settings.hook).indexOf(theRel) ? $(n).attr("href") :void 0;
}) :$.makeArray($(this).attr("href")), pp_titles = isSet ? jQuery.map(matchedObjects, function(n) {
return -1 != $(n).attr(settings.hook).indexOf(theRel) ? $(n).find("img").attr("alt") ? $(n).find("img").attr("alt") :"" :void 0;
}) :$.makeArray($(this).find("img").attr("alt")), pp_descriptions = isSet ? jQuery.map(matchedObjects, function(n) {
...
return "undefined" == typeof settings && (settings = pp_settings, $.browser.msie && 6 == $.browser.version && (settings.theme = "light_square"), 
pp_images = $.makeArray(arguments[0]), pp_titles = arguments[1] ? $.makeArray(arguments[1]) :$.makeArray(""), 
pp_descriptions = arguments[2] ? $.makeArray(arguments[2]) :$.makeArray(""), isSet = pp_images.length > 1 ? !0 :!1, 
set_position = arguments[3] ? arguments[3] :0, _build_overlay(event.target)), $.browser.msie && 6 == $.browser.version && $("select").css("visibility", "hidden"), 
...
movie_id.indexOf("&") > 0 && (movie_id = movie_id.substr(0, movie_id.indexOf("&")))), 
movie = "http://www.youtube.com/embed/" + movie_id, movie += getParam("rel", pp_images[set_position]) ? "?rel=" + getParam("rel", pp_images[set_position]) :"?rel=1", 
...
currentGalleryPage = 0, "previous" == direction ? (set_position--, 0 > set_position && (set_position = $(pp_images).size() - 1)) :"next" == direction ? (set_position++, 
...
0 > currentGalleryPage && (currentGalleryPage = totalPage)) :currentGalleryPage = direction, 
...
$.browser.msie && 6 == $.browser.version && $("select").css("visibility", "visible"), 
