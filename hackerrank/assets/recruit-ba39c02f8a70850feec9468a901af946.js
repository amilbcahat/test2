...
h.design = "Design", h.code_upload = "Code upload", h[key] ? h[key] :key;
...
}, RecruitController.prototype.logicalTemplatePath = function(template) {
...
}(window.HR.CommonController), HR = null != (_ref = window.HR) ? _ref :{}, HR.RecruitController = RecruitController;
});
}.call(this), function() {
...
return this.navigate = __bind(this.navigate, this), RecruitRouter.__super__.constructor.apply(this, arguments);
}
...
"tests/search":"testsSearch",
"tests/search/":"testsSearch",
"tests/search/:filters":"testsSearch",
...
"tests/:id/attempts/invite":"inviteCandidates",
"library/questions/new/:type":"createLibraryQuestion",
"library/questions/:qid/edit(/:step)":"editLibraryQuestion",
"library/questions/:qid/try":"tryLibraryQuestion",
library:"library",
"library/:library":"library",
"library/:library(/:access)/:filter":"library",
"tests/:id/library":"testLibrary",
"tests/:id/library/:library":"testLibrary",
"tests/:id/library/:library/:filter":"testLibrary",
"tests/:id/attempts/:aid/report":"testReport",
"tests/:id/attempts/:aid/report/:tab":"testReport",
"tests/:id/attempts/:aid/report/:tab/:qid":"testReport",
...
"tests/:id/attempts/:type":"reports",
"tests/:id/attempts/:type/:subtype":"reports",
...
"interviews/attempts":"interviewCandidates",
"interviews/attempts/:email":"interviewCandidate",
...
}), re_match = urlhash.match(/tests\/(\d*)\/attempts\/(\d*).*/), re_match && 3 === re_match.length ? (target_url = "/" + urlhash, 
...
void 0) :(re_match = urlhash.match(/report\/(\d*)\/(\d*).*/), re_match && 3 === re_match.length ? (target_url = "/tests/" + re_match[1] + "/attempts/" + re_match[2] + "/report", 
queryDict.authkey && "" !== queryDict.authkey && (target_url += "?authkey=" + queryDict.authkey), 
...
}, RecruitRouter.prototype.tests = function(page, filters, advanced_search_open) {
return null == page && (page = 1), null == filters && (filters = ""), null == advanced_search_open && (advanced_search_open = !1), 
HR.requires([ "compound/x/recruit-basic" ], function() {
var render_view, tests;
return HR.appController.setTitle("Tests"), tests = new HR.TestsCollection(), tests.setPage(page), 
HR.search || (HR.search = {}), HR.search.tests || (HR.search.tests = new HR.SearchItemsCollection()), 
HR.search.tests.setType("tests"), render_view = function() {
...
}(this), "" !== filters ? (render_view(tests), void 0) :HR.PREFETCH_DATA.tests && HR.PREFETCH_DATA.tests.models ? (tests.reset(HR.PREFETCH_DATA.tests.models), 
...
var test;
return test = "downloadnoans" === action ? new HR.RecruitTestModel({
id:id
}) :new HR.TestModel({
id:id
}), test.fetch({
...
return HR.appController.setTitle(model.get("name")), track_data = {
Page:"Questions"
}, HR.util.track("Viewed Test Settings", track_data), test_questions_view = new HR.TestQuestionsView({
model:model,
action:action
}), "download" === action || "downloadnoans" === action ? (HR.appView.setTopNavigationView(model), 
HR.appView.setFullContentView(test_questions_view)) :HR.appView.setSplitContentView("test", model, "questions", test_questions_view);
};
}(this),
error:function() {
return console.log("error");
...
return HR.appController.setTitle(model.get("name")), HR.emailqueue ? (ems = HR.emailqueue, 
HR.emailqueue = null) :ems = [], test_invite_view = new HR.TestInviteView({
model:model,
emails:ems
...
}, RecruitRouter.prototype.library = function(lib, access, filter) {
return null == lib && (lib = null), null == filter && (filter = null), HR.requires([ "compound/x/recruit-basic" ], function() {
var lib_view, track_data;
return HR.appController.setTitle("Library"), "coding" !== filter && "multiple" !== filter && "others" !== filter && (access = filter, 
filter = null), null === lib && (lib = window.lastLib ? window.lastLib :"hackerrank"), 
null === filter && (filter = window.lastFilter ? window.lastFilter :"coding"), lib_view = new HR.LibraryView({
library:lib,
filter:filter,
access:access
}), null != access && (lib = "" + lib + "_" + access), HR.appView.setSplitContentView("library", null, lib, lib_view), 
track_data = {
page:lib
}, HR.util.track("Viewed Library", track_data);
});
}, RecruitRouter.prototype.createLibraryQuestion = function(type, qid, step) {
return null == qid && (qid = null), null == step && (step = "step1"), HR.requires([ "compound/x/recruit-basic" ], function() {
...
type:type,
isLibrary:!0
}), HR.appView.setSplitContentView("library", null, "personal_mine", question_create_view);
});
}, RecruitRouter.prototype.editLibraryQuestion = function(qid, step) {
return null == step && (step = "step1"), this.createLibraryQuestion(null, qid, step);
}, RecruitRouter.prototype.tryLibraryQuestion = function() {}, RecruitRouter.prototype.testLibrary = function(tid, lib, filter) {
...
}, RecruitRouter.prototype.testsSearch_nofilter = function(page) {
return null == page && (page = 1), this.tests(page, "", !0);
}, RecruitRouter.prototype.testsSearch = function(filters, page) {
var temp;
return null == filters && (filters = ""), null == page && (page = 1), "" === filters && (filters = window.location.search), 
filters.indexOf("page=") >= 0 && (temp = filters.split("page="), filters = temp[0], 
page = temp[1]), this.tests(page, filters.split("=").join("|"), !0);
...
}, RecruitRouter.prototype.apiSettings = function() {
return HR.requires([ "compound/x/recruit-basic" ], function() {
var api_authorized_clients, api_clients, api_tokens;
return api_clients = new HR.ApiClientsCollection(), api_tokens = new HR.ApiTokensCollection(), 
api_authorized_clients = new HR.ApiAuthorizedClientsCollection(), api_clients.fetch({
success:function() {
return function() {
return api_tokens.fetch({
success:function() {
return api_authorized_clients.fetch({
success:function() {
var settings_view, track_data;
return track_data = {
Page:"API Settings"
}, HR.util.track("Viewed Account Settings", track_data), settings_view = new HR.SettingsApiView({
clients:api_clients,
tokens:api_tokens,
authorized_clients:api_authorized_clients
}), HR.appView.setSplitContentView("settings", null, "api", settings_view);
},
error:function(m, response) {
return 401 === response.status ? HR.util.ajaxmsg(response.responseText.message, !1, !0, 1e3) :console.log("error fetching authorized clients collection");
}
});
},
error:function(m, response) {
return 401 === response.status ? HR.util.ajaxmsg(response.responseText.message, !1, !0, 1e3) :console.log("error fetching tokens collection");
}
});
};
}(this),
...
return 401 === response.status ? HR.util.ajaxmsg(response.responseText.message, !1, !0, 1e3) :console.log("error fetching client collection");
};
}(this)
});
});
...
error:function(m, response) {
return 401 === response.status ? HR.util.ajaxmsg(response.responseText.message, !1, !0, 1e3) :console.log("error fetching team collection");
}
});
});
...
}, RecruitRouter.prototype.navigate = function(fragment, options) {
var answer;
return answer = $.Deferred(), answer.promise().then(function(_this) {
return function() {
return RecruitRouter.__super__.navigate.call(_this, fragment, options);
};
}(this)), HR.appView && HR.appView.dirty ? HR.Vent.trigger("view:dirty", answer) :answer.resolve();
}, RecruitRouter.prototype.beforeUnload = function() {
return HR.appView && HR.appView.dirty ? "You have unsaved changes. To stay on the page, press Cancel. To discard changes and leave the page, Press OK" :void 0;
}, RecruitRouter;
...
h.uml = "UML", h.electrical = "Electrical", h.design = "Design", window.istreet.cfg.ats = new Object(), 
...
var accessToken;
return accessToken = $.cookie("access_token"), accessToken ? xhr.setRequestHeader("Authorization", "Bearer " + accessToken) :void 0;
...
HR.router = new HR.RecruitRouter(), HR.Vent = _.clone(Backbone.Events), $(window).on("beforeunload", HR.router.beforeUnload), 
